...
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/player.html')) {
    ...
    var MAssist = (function () {
        ...
        var countDown, contributors = [], isCountDownActive, isGameActive, tipRegex, alertTimeout, highestTip = 0, stopRegex, UpdateTopic, lastRegex, guessPrizeRegex;
        ...
        var mfcLoadPlayer, TwitMsg, VidMsg, gameRegex, chanceGuessed = [], chance, addgameRegex, toggleGuessRegex, highTipRegex, addHighRegex, bannedRemoveRegex;
        var guessStartRegex, guessStopRegex, warStartRegex, warStopRegex, countrRegex, gamearray, isWarActive, wartip, VarVid, topicVar2 = false, nopeRegex, BlkJk2Regex;
        var assignWarRegex, ModelCardVal = [], addBannedRegex, topicValRegex, SpecialRegex, ShowRegex, Show1, Show2, Show3, Show4, Show5, toggleNoRegex, postNo,BlkJkPrizeRegex;
        var PlayerCardVal = [], ModelSuitVal = [], PlayerSuitVal = [], playWarRegex, CardVal = [], isHelperAssigned, assignHelperRegex, removeHelperRegex, BlkJk1Regex;
        var warPrizeAssigned, guessPrizeAssigned, warPrize, guessPrize, stopGamesRegex, helpers = [], ParkMsg, guessPrizeFund = [], warPrizeFund = [], banned = [], BlkJkPrizeFund = [];
        var DealerSet, BlkJkPrizeSet, BlkJkRd1, PlayerHand, DealerHand, BlkJkRd2, PlayerHand2, DealerHand2, BlkJkStopRegex, Dealer, Player, BlkJkWinner, BlkJkPrize, BlkJKWinnerRegex;
        function init() {
            setTimeout(function () {
        ...
        function setupHTML() {
            ...
            var txtMenuText = document.createTextNode('AnaliasCountr Chat V2');
        ...
        gamearray = ["guessing game", " war game, BlackJack"];
        ...
        function toggleSpecials(SVar, msgSpan) {
            ...
            if (nameSpan) {
                ...
                poster = postie.innerHTML.replace(/PM from/,'');
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter") || (poster == "HoleeeSheeet")) {
                    var val = SVar[1];
                    var control = val.toLowerCase();
                    var ctrlval = parseInt(SVar[2]);
                    var ctrlvaltxt = SVar[3];
                    if (control == "spank"){
                        SpankSpec = ctrlval;
                        if (SpankSpec > 0) {
                        ...
                        VidTrp = ctrlval;
                        if (VidDbl > 0){
                            postChatMessage("Non topic Vid value has been set to " + VidDbl);
                        }
                        else {
                            postChatMessage("Non topic Vid value has been set to default " + VidDbl)
                        }
                    }
        ...
        function postCurrentCount() {
            ...
            VidMsg = "all videos listed on http://profiles.myfreecams.com/$$model$$    :doa15".replace("$$model$$", currentModelName);
            if (countDown >= (1+T2)){
        ...
        function parseChatMessage() {
            ...
            while (parseQueue.length > 0) {
                ...
                //StopBlackJack?
                var StopBlkJk = msg.innerHTML.match(BlkJkStopRegex);
                if (StopBlkJk)
                    ClearBlkJack(msg);
                //BlackJack Winner?
                var win = msg.innerHTML.match(BlkJkWinner);
                if (win)
                    compileWinner(msg);
                // startregex?
                ...
                if (nada)
                    postNope(msg);
                //BlackJack Prize Regex?
                var BlkJkPrz = msg.innerHTML.match(BlkJkPrizeRegex);
                if (BlkJkPrz)
                    assignBlkJkPrize(blkjkPrz, msg);
        ...
        function compileWinner(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                ...
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster === helper) || (poster == "TestCounter") || (poster == "HoleeeSheeet")) {
                    if (DealerHand2 > PlayerHand2) {
                        postChatMessage(Dealer + " wins with " + DealerHand2 + " over " + PlayerHand2);
                    }
                    else if (DealerHand2 === PlayerHand2) {
                        postChatMessage("Draw!!, playing again");
                        DealerSet = false;
                        BlkJkRd1 = false;
                        BlkJkRd2 = false;
                        PlayerHand = 0;
                        ...
                        if (BlkJkPrizeSet) {
                            postChatMessage(Player + " wins " + BlkJkPrize)
                        }
                        else {
                            postChatMessage(Player + " wins");
                        }
                    }
                }
            }
        ...
        function play1stHand(BlkJk1, msgSpan) {
            ...
            if (nameSpan) {
                ...
                if ((poster == "Parkesy86") || (poster == "DreamOfAnalia") || (poster === helper) || (poster == "TestCounter") || (poster == "HoleeeSheeet")) {
                    ...
                    var Val1Mod = Math.floor((Math.random() * 13), 0);
                    //Card value 2 for Model
                    var Val2Mod = Math.floor((Math.random() * 13), 0);
                    //Card value 1 for Player
                    var Val1P = Math.floor((Math.random() * 13), 0);
                    //Card value 2 for Player
                    var Val2P = Math.floor((Math.random() * 13), 0);
                    //Get the hand values
                    ...
                    if (BlkJkRd1) {
                        ...
                        if ((PlayerHand === 0) || (DealerHand === 0)) {
                            ...
                            if (cardchoice == "player") {
                                //Card value 1 for Player
                                var Val4P = Math.floor((Math.random() * 13), 0);
                                //Get the hand values
                                PlayerHand2 = Val4P + PlayerHand;
                            ...
                            if (cardchoice == "player") {
                                if (poster === Player) {
                                    //Card value 3 for Player
                                    var Val3P = Math.floor((Math.random() * 13), 0);
                                    //Get the hand values
                                ...
                                var Val3Mod = Math.floor((Math.random() * 13), 0);
                                //Get the hand values
        ...
        function ClearBlkJack(msgSpan) {
            ...
            if (nameSpan) {
                ...
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter") || (poster == "HoleeeSheeet") || (poster === helper)) {
                    ...
                    PlayerHand2 = 0;
                    DealerHand2 = 0;
                    postChatMessage("BlackJack values have been cleared");
                }
            }
        }
        ...
        function assignBlkJkPrize(blkjkPrz, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                BlkJkPrize = blkjkPrz[1];
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter") || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (guessPrizeAssigned) {
                        postChatMessage(poster + " prize for winning blackjack game has been updated successfully to: " + BlkJkPrize);
                        BlkJkPrizeSet = true;
                        BlkJkPrizeFund.push(BlkJkPrize);
                        ParkMsg = BlkJkPrizeFund;
                        postToPark(ParkMsg);
                    }
                    else {
                        postChatMessage(poster + " guessing game prize has now been assigned as: " + BlkJkPrize);
                        BlkJkPrizeSet = true;
                        BlkJkPrizeFund.push(BlkJkPrize);
                        ParkMsg = BlkJkPrizeFund;
                        postToPark(ParkMsg);
                    ...
                    postChatMessage("I'm sorry " + poster + " only authorised people can assign the guess prize");
                    ParkMsg = poster + " tried to assign guess prize as " + BlkJkPrize;
                    postToPark(ParkMsg);
                }
            }
        }
        ...
        function postTopic(msgSpan) {
            ...
            if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter") || (poster === helper)) {
                if (isCountDownActive) {
                    if (topicVar2) {
                        if (countDown >= (1 + T2)) {
                            postChatMessage("Hi Guys! " + (countDown - T2) + " Nakkey, " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T3)) {
                            postChatMessage("Hi Guys! " + (countDown - T3) + Show1 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + " vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T4)) {
                            postChatMessage("Hi Guys! " + (countDown - T4) + Show2 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + " vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T5)) {
                            postChatMessage("Hi Guys! " + (countDown - T5) + Show3 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + " vid "+ VidSgl + ", TY Guys")
                        }
                        else {
                        ...
                        if (countDown >= (1 + T2)) {
                            postChatMessage("Hi Guys! " + (countDown - T2) + show4 + " Off, " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + "Vids " + VidDbl + "(choose from profile) " + VarVid + " vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T3)) {
                            postChatMessage("Hi Guys! " + (countDown - T3) + show5 + " Off, " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + " vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T4)) {
                            postChatMessage("Hi Guys! " + (countDown - T4) + Show1 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + "vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T5)) {
                            postChatMessage("Hi Guys! " + (countDown - T5) + Show2 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + " vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= 1) {
                            postChatMessage("Hi Guys! " + countDown + Show3 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + " vid " + VidSgl + ", TY Guys")
                        }
                    }
                }
            }
        ...
        function TopicX(posty) {
            if ((posty == "DreamOfAnalia") || (posty == "Parkesy86") || (posty == "TestCounter") || (poster === helper)) {
                if (isCountDownActive) {
                    if (topicVar2) {
                        if (countDown >= (1 + T2)) {
                            postChatMessage("Hi Guys! " + (countDown - T2) + " Nakkey, " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + " Vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T3)) {
                            postChatMessage("Hi Guys! " + (countDown - T3) + Show3 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile)" + VarVid + " Vid " + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T4)) {
                            postChatMessage("Hi Guys! " + (countDown - T4) + Show4 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys")
                        }
                        else if (countDown >= (1 + T5)) {
                            postChatMessage("Hi Guys! " + (countDown - T5) + Show5 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys")
                        }
                        else {
                        ...
                        if (countDown >= (1 + T2)) {
                            postChatMessage("Hi Guys! " + (countDown - T2) + Show1 + " Off, " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys");
                        }
                        else if (countDown >= (1 + T3)) {
                            postChatMessage("Hi Guys! " + (countDown - T3) + Show2 + " Off, " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys");
                        }
                        else if (countDown >= (1 + T4)) {
                            postChatMessage("Hi Guys! " + (countDown - T4) + Show3 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys");
                        }
                        else if (countDown >= (1 + T5)) {
                            postChatMessage("Hi Guys! " + (countDown - T5) + Show4 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys");
                        }
                        else if (countDown >= 1) {
                            postChatMessage("Hi Guys! " + countDown + Show5 + " show, " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(choose from profile) " + VarVid + VidSgl + ", TY Guys");
                        }
                    }
                }
            }
        ...
        function removeSpam(msgSpan) {
            ...
            if (nameSpan) {
                ...
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter") || (poster == "HoleeeSheeet") || (poster === helper) || (poster == "AnaliasPrince") || (poster == "AnaliasWes") || (poster == "AnaliasPony") || (poster == "")) {
                    ...
                    postChatMessage(".");
                    postChatMessage(".");
                    postChatMessage("spam removed as requested " + poster);
                }
            }
        }
        ...
        function modelChanged(modelId) {
            ...
            guessStartRegex = /start guess! game/i;
            guessStopRegex = /stop guess! game/i;
            addgameRegex = /add (.*) to numbers!/i;
            lastRegex = /Topic: (.[^:]+) TY Guys/i;
            assignWarRegex = /add (.*) as war! tip/i;
            warStartRegex = /start war!/i;
            warStopRegex = /stop war!/i;
            BlkJkStopRegex =/stop blackjack!/i;
            ...
            warPrizeRegex = /war! prize is (.[^:]+)/i;
            guessPrizeRegex = /guess! prize is (.[^:]+)/i;
            BlkJkPrizeRegex = /blackjack! prize is (.[^:]+)/i;
            ...
            BlkJk1Regex = /blackjack (.[^:]+) x/i;
            BlkJk2Regex = /blackjack draw (player|house)/i;
            BlkJKWinnerRegex = /blackJack winner!/i;
            guessPrizeFund = [];
            VidDbl = 555;
            SpankSpec = 99;
            VidSgl = 500;
