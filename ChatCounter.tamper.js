...
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/player.html')) {
    ...
    var MAssist = (function () {
        ...
        var countDown, contributors = [], isCountDownActive, isGameActive, tipRegex, alertTimeout, highestTip = 0, stopRegex, UpdateTopic, lastRegex;
        var parsing = false, parseQueue = [], autoPostActive = false, postCountTimeout = null, killPosting = false, getlastRegex, spamRegex;
        ...
        var mfcLoadPlayer, Controller, PMValue, TwitMsg, VidMsg, gameRegex, chanceGuessed = [], chance, guessChangeRegex, addgameRegex, toggleGuessRegex;
        var guessStartRegex, guessStopRegex, warStartRegex, warStopRegex, countrRegex, gamearray, isWarActive, wartip, assignWarRegex, ModelCardVal = [];
        var PlayerCardVal = [], ModelSuitVal = [], PlayerSuitVal = [], playWarRegex, CardVal = [];;
        function init() {
            setTimeout(function () {
        ...
        function setupHTML() {
            ...
            var txtMenuText = document.createTextNode('AnaliasCountr Chat');
        ...
        Vid7 = 0;
        Vid8 = 0;
        gamearray = ["guessing game", " war game"];
        ...
        function postCurrentCount() {
            ...
            PMValue = "/pm $$model$$ ".replace("$$model$$", currentModelName);
            TwitMsg = "make sure to follow $$model$$ on Twitter :doatwit".replace("$$model$$", currentModelName);
            VidMsg = "all videos listed on http://profiles.myfreecams.com/$$model$$".replace("$$model$$", currentModelName);
            if (countDown >= T1) {
                postChatMessage(TwitMsg);
                postChatMessage(VidMsg);
                postChatMessage("/pm DreamOfAnalia topics set to:  " + Topic1 + ", " + Topic2 + ", " + Topic3 + ", " + Topic4 + ", " + Topic5);
                postChatMessage("/pm DreamOfAnalia topic specials set to " + TpcSpec1 + ", " + TpcSpec2 + ", " + TpcSpec3 + ", " + TpcSpec4 + ", " + TpcSpec5 + ", " + TpcSpec6);
                postChatMessage("/pm DreamOfAnalia for 2555: Hi Guys! 2555tk Bra Off, 99tk Spank/100tk Flash, 1717tk SnaChat 4ever, Vid Sale 4 Vids 999tk(u choose from profile) G/G vid 500tk, TY Guys!");
            }
                ...
                postChatMessage("no countdown running, please contact the countdown user or use chat commands to start a new one");
            }
        ...
        function clearGame(msgSpan) {
            ...
            if (nameSpan) {
                ...
                if ((currentModelName === poster) || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    chanceGuessed.length = 0;
                    postChatMessage("game data has been reset by " + poster);
                }
                else {
                    postChatMessage(":nono I'm sorry " + poster + " you can not remove game data");
                }
            }
        }
            ...
            if (nameSpan) {
                ...
                if ((currentModelName === poster) || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    if (isGameActive) {
                        ...
                        isGameActive = true;
                        postChatMessage("Guessing game has been started, tip a value between 1-100, if it matches you win a prize");
                    }
                }
                else {
                    postChatMessage("");
                ...
                postChatMessage(":mno " + poster + " you are not allowed to change the guess number");
            }
        }
        function stopGuess(msgSpan) {
            ...
            if (nameSpan) {
                ...
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    if (isGameActive) {
                        postChatMessage("I am stopping the guessing game as requested & setting value to 0 " + poster);
                        isGameActive = false;
                        chance = 0;
                    }
                    else {
                    ...
                    postChatMessage("I'm sorry " + poster + " you are not allowed to start/stop the game");
                }
            }
        }
                ...
                if ((currentModelName === poster) || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    if (wartip === 0){
                        ...
                        if (isWarActive) {
                            ...
                            isWarActive = true;
                            postChatMessage("war game has been initialised, tips of: " + wartip + " (no multiples) play war with the model");
                        }
                    }
                }
            }
        ...
        function stopWar(msgSpan) {
            ...
            if (nameSpan) {
                ...
                if ((currentModelName === poster) || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    if (isWarActive){
                        postChatMessage("war game has been stopped " + poster);
                        isWarActive = false;
                    }
                    else {
        ...
        function manualWar(msgSpan, play) {
            ...
            if (nameSpan) {
                ...
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    ...
                    postChatMessage("I'm sorry only " + currentModelName + " can manually play war");
                }
            }
        }
        function postTopic(msgSpan) {
            ...
            if (nameSpan) {
                ...
                if ((currentModelName === poster) || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    if (isCountDownActive) {
                        if (countDown >= T1) {
                            postChatMessage("unfortunately the countdown doesn't match my pre-determined values, therefore I can not provide a topic");
                        }
                        else if (countDown >= (1 + T2)) {
                            ...
                            postChatMessage("Happy Monday Guys! $$value$$ [type] show, 99tk Spank, 1717tk SnapChat 4ever, 2 Vids 555tk(choose from profile) G/G Vid 500tk, TY Guys").replace("$$value$$", CDVal4);
                        }
                        else {
                            var CDVal5 = (countDown);
                            postChatMessage("Happy Monday Guys! $$value$$ [type] show, 99tk Spank, 1717tk SnapChat 4ever, 2 Vids 555tk(choose from profile) G/G Vid 500tk, TY Guys").replace("$$value$$", CDVal5);
                        }
                    }
                    else {
                        postChatMessage("I'm sorry " + poster + " no countdown is running");
                    }
                }
            }
        }
        function postCountrInfo(msgSpan) {
            ...
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((currentModelName === poster) || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    postChatMessage("currently running countr which covers topics " + Topic1 + ", " + Topic2 + ", " + Topic3 + ", " + Topic4 + ", " + Topic5);
                    postChatMessage("countr also includes the following games: " + gamearray.sort() + " if you want help type 'countr! help'");
                }
            }
        }
        ...
        function updatedTopic(topic, topicSpan) {
            if (topicSpan.getAttribute('class').indexOf('chat_system') === -1) return;
            //parse the values
            ...
            if (nameSpan) {
                ...
                if ((currentModelName === poster) || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    if (isWarActive) {
                        ...
                        postChatMessage("war amount is: " + wartip);
                    }
                    if (isGameActive) {
                    ...
                    if (isGameActive) {
                        postChatMessage("DreamOfAnalia's Random Number Game, simply tip an amount between 1-100 and if it matches the random number you win (max 1 person per game)");
                        postChatMessage("numbers guessed so far are: " + chanceGuessed.sort());
                        postChatMessage("/pm DreamOfAnalia the random number for today is " + chance + " whoever tips this wins (max 1 person per game)");
                    }
                    else {
                        postChatMessage("no games are currently running");
                    }
                }
            }
        }
        ...
        function postHelp(msgSpan) {
            ...
            if (nameSpan) {
                ...
                postChatMessage("Following commands are used for countr: minus xxx | plus xxx | topic please | start topic x | countr help");
                    ...
                    postChatMessage("I'm sorry " + poster + " I can't help you :giggle4");
                }
            }
        }
            ...
            if (amount <= 100) {
                if (isGameActive) {
                    if (amount === chance) {
                        ...
                        postChatMessage(PMValue + contributor + " has won the guessing game with the value " + amount);
        ...
        function adjustTopicSpecials(amount, contributor) {
            if (amount === TpcSpec1) {
                ...
                postChatMessage("/pm DreamOfAnalia nothing set for this total");
            }
        }
        ...
        function modelChanged(modelId) {
            ...
            minusRegex = "minus! (.*)";
            plusRegex = "plus! (.*)";
            postRegex = "topic! please";
            startRegex = "start! topic (.*)";
            helpRegex = "countr! help";
            stopRegex = "countr! stop";
            gameRegex = "game! is";
            countrRegex = "which version! countr?";
            toggleGuessRegex = "clear guess! game";
            guessStartRegex = "start guess! game";
            guessStopRegex = "stop guess! game";
            addgameRegex = "add (.*) to numbers! guessed";
            lastRegex = "Topic: (.[^:]+) TY Guys";
            assignWarRegex = "add (.*) as war! tip";
            warStartRegex = "start war! game";
            warStopRegex = "stop war! game";
            playWarRegex = "play war! (.[^:]+) vs me";
