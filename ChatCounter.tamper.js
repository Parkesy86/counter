...
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/player.html')) {
    ...
    var MAssist = (function () {
        ...
        var mfcLoadPlayer, PMValue, TwitMsg, VidMsg, gameRegex, chanceGuessed = [], chance, addgameRegex, toggleGuessRegex, highTipRegex, addHighRegex, bannedRemoveRegex;
        var guessStartRegex, guessStopRegex, warStartRegex, warStopRegex, countrRegex, gamearray, isWarActive, wartip, assignWarRegex, ModelCardVal = [], addBannedRegex;
        var PlayerCardVal = [], ModelSuitVal = [], PlayerSuitVal = [], playWarRegex, CardVal = [], isHelperAssigned, assignHelperRegex, removeHelperRegex;
        ...
        function postCurrentCount() {
            ...
            if (countDown >= T1) {
                ...
                postChatMessage("/pm DreamOfAnalia for 2555: Hi Guys! 2555tk Bra Off, 99 Spank/100 Flash, 1717tk SnaChat 4ever, Vid Sale 4 Vids 999tk(u choose from profile) G/G vid 500tk, TY Guys!");
            }
                ...
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter") || (poster === helper)) {
                    if (isCountDownActive) {
                        if (countDown >= T1) {
                            ...
                            postChatMessage("Hi Guys! $$value$$ Bra Off, 99 Spank/100 Flash, 1717tk SnapChat 4ever, 2 Vids 555tk(choose from profile) G/G Vid 500tk, TY Guys").replace("$$value$$", CDVal1);
                        }
                        else if (countDown >= (1 + T3)) {
                            var CDVal2 = (countDown - T3);
                            postChatMessage("Hi Guys! $$value$$ Panties Off, 99 Spank/100 Flash, 1717tk SnapChat 4ever, 2 Vids 555tk(choose from profile) G/G Vid 500tk, TY Guys").replace("$$value$$", CDVal2);
                        }
                        else if (countDown >= (1 + T4)) {
                            ...
                            postChatMessage("Hi Guys! $$value$$ [type] show, 99tk Spank, 1717tk SnapChat 4ever, 2 Vids 555tk(choose from profile) G/G Vid 500tk, TY Guys").replace("$$value$$", CDVal5);                        }
                    }
                    else {
        ...
        function removeSpam(msgSpan) {
            ...
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster == "DreamOfAnalia") || (poster == "Parkesy86") || (poster == "TestCounter") || (poster === helper)) {
                    postChatMessage("removing spam as requested " + poster);
                    postChatMessage(".");
                    postChatMessage(".");
                    postChatMessage(".");
                    postChatMessage(".");
        ...
        function modelChanged(modelId) {
            ...
            stopGamesRegex = "stop all! games";
            highTipRegex = "highest! tip?";
            playWar2Regex = "war! for two (.[^:]+) vs (.[^:]+)";
            playWarRegex = "play war! (.[^:]+) me";
            getlastRegex = "my topic!";
            spamRegex = "!spam!";
            addHighRegex = "(.[^:]+) with (.*) highest tip";
            warPrizeRegex = "war! prize is (.[^:]+)";
            guessPrizeRegex = "guess! prize is (.[^:]+)";
            assignHelperRegex = "helper is (.[^:]+) please";
            removeHelperRegex = "remove (.[^:]+) from helper";
            checkHelperRegex = "!who is helper!";
            bannedRemoveRegex = "remove! (.[^:]+) from ban list";
            addBannedRegex = "add (.[^:]+) to countr! ban";
            guessPrizeFund = [];
            warPrizeFund = [];
            isHelperAssigned = false;
            warPrizeAssigned = false;
