// ==UserScript==
// @name AnaliasCountr Chat V5.10
// @version	Chat 5.10
// @description	script to be used only in Model's room
// @include	http://www.myfreecams.com/*
// @match http://www.myfreecams.com/*
// ==/UserScript==
//polyfill for unsafeWindow on Chrome
if (navigator.userAgent.match(/chrome/i)) {
    unsafeWindow = (function () {
        var el = document.createElement('p');
        el.setAttribute('onclick', 'return window;');
        return el.onclick();
    } ());
}
//for some reason @match doesn't appear to work properly in Chrome, so check it here
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/player.html')) {
    var maStyles = [
        '    #ma-alert ',
        '    {',
        '    	    position: absolute; ',
        '    	    top: 2px; ',
        '    	    width: 35%; ',
        '    	    height: 30px; ',
        '    	    left: 0; ',
        '    	    right: 0; ',
        '    	    margin-left: auto; ',
        '    	    margin-right: auto; ',
        '    	    background: White; ',
        '    	    text-align: center; ',
        '    	    font-family: Trebuchet MS; ',
        '    	    -moz-border-radius: 10px; ',
        '    	    -webkit-border-radius: 10px;',
        '    	    line-height:1.75em; ',
        '    	    display:none;',
        '    } ',
        '    .processed {color: #66CCFF; font-style:italic} ',
        '    .hidden {display: none;} ',
        '    .show {display:block}',
        '   #ma-menu ',
        '    {',
        '        position:absolute; ',
        '        top:0; ',
        '        right:0;',
        '        background-color:White;',
        '        -moz-border-radius-bottomleft: 10px; ',
        '        -webkit-border-bottom-left-radius: 10px; ',
        '        width: 200px;',
        '        font-family: Trebuchet MS;',
        '        text-align: center; ',
        '        z-index:1001;',
        '    } ',
        '    #ma-menu:hover #ma-nav {display:block;} ',
        '    #ma-nav {display:none;padding:0;margin:0;text-align:left;list-style:none;font-size: smaller;padding: 2px 10px 10px;}',
        '    #ma-menu table {width:100%; border-collapse:collapse}',
        '    #ma-menu table td {font-size:x-small}',
        '    #ma-menu a {text-decoration:underline; color:black; cursor: pointer}',
        '    #ma-menu select {font-size: x-small; display: inline}',
        '    #ma-menu hr {padding:0;margin:3px}'
    ];
    GM_addStyle(maStyles.join('\n'));
    var MAssist = (function () {
        var htmlSetup = false, currentModelID = "", findPlayer, currentModelName, killRegex, startRegex, minusRegex, plusRegex, postRegex, helpRegex;
        var countDown, contributors = [], isCountDownActive, tipRegex, alertTimeout, highestTip = 0, stopRegex;
        var parsing = false, parseQueue = [], autoPostActive = false, postCountTimeout = null, killPosting = false, getlastRegex, spamRegex, checkHelperRegex;
        var spanHighestTip, spanCurrentModel, spanMenuCount, spanTopContrib, divAlert, txtInput, btnSend, aAutoPost, aPostCurrentCount, soundDiv;
        var Topic1, Topic2, Topic3, Topic4, Topic5, TipHerRegex;
        var mfcLoadPlayer, highTipRegex, addHighRegex, bannedRemoveRegex, banned = [], helpers = [];
        var countrRegex, toggleNoRegex, nopeRegex, postNo, ParkMsg, ModelMsg, TipReq, CountrSpec, CountrSpec1, CountrSpec2, CountrSpec3;
        var topicValRegex, ShowRegex, Show1, Show2, Show3, Show4, Show5, Vid1, Vid2, Vid3, Vid4, Vid5, Vid6, Vid7, Vid8, RoomMsg;
        var SpecialRegex, SpankSpec, FlashSpec, VidTrp, VarVid, VidAmt, SnapSpec, VidSgl, VidDbl, ChangePostingRegex;
        var isHelperAssigned, assignHelperRegex, removeHelperRegex, addBannedRegex, StopTopicRegex, noTopicRegex, PvtRegex, helper = '';
        var PostTopSpec, TestModeRegex, spoofTipRegex, TestMode, QuestionAmount, RequestRegex, AdjustCountRegex, PostActive = true;
        var startGameRegex, stopGamesRegex, setGamePrizeRegex, setGameTipRegex, addgameRegex, gameRegex, gamearray;
        var GuessPrize, chance, GuessPrizeAssigned, isGuessActive, toggleGuessRegex, chanceGuessed = [], PostTips = false;
        var SpinRegex, isSpinActive, SpinPrize, SpinTip, SpinPrizeAssigned, CountrRegex, SaidHi = [], postTipRegex, AlternateCount;
        var CardSuits, ModelCardLocation, PlayerCardLocation, WarPlayer, WarTip, playWarRegex, WarPrizeAssigned, WarPrize, isWarActive;
        var BlackJackPrize, BlkJk1Regex, BlkJk2Regex, NewModelHand, NewPlayerHand, BlackJackTip, AskedModelOnce, BlackJackPlayer;
        var ModelSet, PlayerSet, PlayerHand, ModelHand, isGameInProgress, BlackJackPrizeAssigned, isBlackJackActive, TopicUpdateRegex;
        var IsFridayActive = false, IsTopic1Active = false, IsTopic2Active = false, IsTopic3Active = false, IsTopic4Active = false, IsTopic5Active = false;

        function init() {
            setTimeout(function () {
                mfcLoadPlayer = unsafeWindow.LoadPlayer;
                unsafeWindow.LoadPlayer = myLoadPlayer;
            }, 500);
            //default sound to enabled
            localStorage['mfca_soundEnabled'] = localStorage['mfca_soundEnabled'] ? localStorage['mfca_soundEnabled'] : 'enabled';
            localStorage['mfca_friendSound'] = localStorage['mfca_friendSound'] ? localStorage['mfca_friendSound'] : 0;
            //fire the modelchanged event if this is a popup
            if (opener) {
                setTimeout(function () {
                    modelChanged(location.search.match(/broadcaster_id=(\d+)/)[1]);
                }, 1000);
            }
        }
        function myLoadPlayer(sAction, hOptions) {
            var modelId = hOptions ? hOptions['broadcaster_id'] : location.search.match(/broadcaster_id=(\d+)/)[1];
            //            try {
            mfcLoadPlayer(sAction, hOptions);
            modelChanged(hOptions ? hOptions['broadcaster_id'] : null);
            //            } catch (e) {
            //                console.log(e);
            //                console.log(sAction);
            //                console.log(hOptions);
            //                //popupwindow
            //                mfcLoadPlayer('', { broadcaster_id: modelId });
            //                modelChanged(modelId);
            //            }
        }
        function setupHTML() {
            if (htmlSetup)
                return;
            //set up the menu header
            var divMenu = document.createElement('div');
            divMenu.setAttribute('id', 'ma-menu');
            var txtMenuText = document.createTextNode('AnaliasCountr Chat V5.10');
            spanMenuCount = document.createElement('span');
            spanMenuCount.setAttribute('id', 'menuCount');
            divMenu.appendChild(txtMenuText);
            divMenu.appendChild(spanMenuCount);
            //set up menu children
            var hr = document.createElement('hr');
            var br = document.createElement('br');
            //create the nav menu
            var divNav = document.createElement('div');
            divNav.setAttribute('id', 'ma-nav');
            divMenu.appendChild(divNav);
            //create the currentModel span
            spanCurrentModel = document.createElement('span');
            spanCurrentModel.setAttribute('id', 'currentModel');
            divNav.appendChild(document.createTextNode('Model: '));
            divNav.appendChild(spanCurrentModel);
            //create set countdown link
            var countdownFieldSet = document.createElement('fieldset');
            var countdownLegend = document.createElement('legend');
            countdownLegend.innerHTML = 'Countdown';
            countdownFieldSet.appendChild(countdownLegend);
            var aSetCountdown = document.createElement('a');
            aSetCountdown.setAttribute('id', 'setCountDown');
            var txtSetCountdown = document.createTextNode('Set Countdown');
            aSetCountdown.appendChild(txtSetCountdown);
            aSetCountdown.addEventListener('click', startCountDown);
            countdownFieldSet.appendChild(aSetCountdown);
            countdownFieldSet.appendChild(br.cloneNode(true));
            //create post current count link
            aPostCurrentCount = document.createElement('a');
            aPostCurrentCount.setAttribute('id', 'postCurrentCount');
            aPostCurrentCount.setAttribute('class', 'hidden');
            var txtPostCurrentCount = document.createTextNode('Post Current Count');
            aPostCurrentCount.appendChild(txtPostCurrentCount);
            aPostCurrentCount.addEventListener('click', postCurrentCount);
            countdownFieldSet.appendChild(aPostCurrentCount);
            //create autopost link
            aAutoPost = document.createElement('a');
            aAutoPost.setAttribute('id', 'autoPost');
            aAutoPost.setAttribute('class', 'hidden');
            var txtAutoPost = document.createTextNode('Disable AutoPost');
            aAutoPost.appendChild(txtAutoPost);
            aAutoPost.addEventListener('click', toggleAutoPost);
            countdownFieldSet.appendChild(aAutoPost);
            divNav.appendChild(countdownFieldSet);
            //create highest tip span
            var statsFieldSet = document.createElement('fieldset');
            var statsLegend = document.createElement('legend');
            statsLegend.innerHTML = 'Stats';
            statsFieldSet.appendChild(statsLegend);
            spanHighestTip = document.createElement('span');
            spanHighestTip.setAttribute('id', 'highestTip');
            //create top contributor span
            spanTopContrib = document.createElement('span');
            spanTopContrib.setAttribute('id', 'topContributor');
            statsFieldSet.appendChild(createTable([['Tip', spanHighestTip], ['Contrib', spanTopContrib]]));
            divNav.appendChild(statsFieldSet);
            //create sound management
            var soundsFieldSet = document.createElement('fieldset');
            var soundsLegend = document.createElement('legend');
            soundsLegend.innerHTML = 'Sounds';
            soundsFieldSet.appendChild(soundsLegend);
            var aToggleSound = document.createElement('a');
            var spanSoundToggle = document.createElement('span');
            spanSoundToggle.setAttribute('id', 'soundToggleText');
            spanSoundToggle.innerHTML = localStorage['mfca_soundEnabled'] == 'enabled' ? 'Disable Sound' : 'Enable Sound';
            aToggleSound.appendChild(spanSoundToggle);
            aToggleSound.addEventListener('click', toggleSound);
            soundsFieldSet.appendChild(aToggleSound);
            var selectFriendSound = document.createElement('select');
            selectFriendSound.innerHTML = '<option value=\'0\'>Model Chime</option><option value=\'1\'>User Chime</option><option value=\'2\'>Tip Chime</option>';
            selectFriendSound.setAttribute('id', 'selectFriendSound');
            selectFriendSound.addEventListener('change', configFriendSound);
            selectFriendSound.selectedIndex = localStorage['mfca_friendSound'] ? localStorage['mfca_friendSound'] : 0;
            selectFriendSound.setAttribute('class', localStorage['mfca_soundEnabled'] == 'enabled' ? 'show' : 'hidden');
            soundDiv = document.createElement('div');
            soundDiv.setAttribute('id', 'soundConfig');
            soundDiv.appendChild(document.createTextNode('Online:  '));
            soundDiv.appendChild(selectFriendSound);
            soundsFieldSet.appendChild(soundDiv);
            divNav.appendChild(soundsFieldSet);
            //create the alert div
            divAlert = document.createElement('div');
            divAlert.setAttribute('id', 'ma-alert');
            //add menu and alert to the body
            var body = document.querySelector('body');
            var chat = document.querySelector("#chat_box");
            chat.insertBefore(divMenu, chat.firstChild);
            body.appendChild(divAlert);
            htmlSetup = true;
        }
        gamearray = [" guessing game", " war game", " BlackJack", " Spin The Wheel"];

        //TestMode
        function InitialiseTestMode(TestModeMsg, msgSpan) {
            var poster;
            var command = TestModeMsg[1];
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if (poster == "TestCounter") {
                    if (command == "on") {
                        TestMode = true;
                        autoPostActive = false;
                        RoomMsg = "Test Mode has been enabled";
                        postToRoom(RoomMsg);
                    }
                    else if (command == "off") {
                        TestMode = false;
                        autoPostActive = true;
                        RoomMsg = "Test Mode has been disabled";
                        postToRoom(RoomMsg);
                    }
                }
                else {
                    RoomMsg = "Test Mode can only be enabled or disabled by TestCounter";
                    postToRoom(RoomMsg);
                }
            }
        }
        function handleTestTip(spoof, msgSpan) {
            if (TestMode) {
                var amount = parseInt(spoof[1]);
                var poster;
                var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
                if (nameSpan) {
                    poster = nameSpan.innerHTML.replace(/:/, '');
                    if (poster == "TestCounter") {
                        var contributor = poster;
                        if (isGuessActive) {
                            PlayGuessGame(amount, contributor);
                            if (isCountDownActive) {
                                adjustCountDown(amount);
                            }
                        }
                        else if (amount === WarTip) {
                            if (isWarActive) {
                                RoomMsg = "shuffling deck, please wait....... :deckshuffle ";
                                postToRoom(RoomMsg);
                                postCountTimeout = setTimeout(function () {
                                    postCountTimeout = null;
                                    StartTipWar(contributor);
                                }, 5000);
                            }
                            if (isCountDownActive) {
                                adjustCountDown(amount);
                            }
                        }
                        else if (amount === BlackJackTip) {
                            if (isBlackJackActive) {
                                if (isGameInProgress) {
                                    RoomMsg = "BlackJack game is in progress, room operator will run a manual game once this one has ended";
                                    postToRoom(RoomMsg);
                                    RoomMsg = "/pm Parkesy86 Please start manual BlackJackgame for " + contributor + " once this one has ended";
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isGameInProgress = true;
                                    var play = contributor;
                                    BlackJackInitialHand(play);
                                }
                                if (isCountDownActive) {
                                    adjustCountDown(amount);
                                }
                            }
                        }
                        else if (amount === SpinTip) {
                            if (isSpinActive) {
                                SpinTheWheel(contributor);
                            }
                            if (isCountDownActive) {
                                adjustCountDown(amount);
                            }
                        }
                        else if (amount === SpankSpec) {
                            adjustTopicSpecials(amount, contributor);
                        }
                        else if (amount === FlashSpec) {
                            adjustTopicSpecials(amount, contributor);
                        }
                        else if (amount === SnapSpec) {
                            adjustTopicSpecials(amount, contributor);
                        }
                        else if (amount === VidSgl) {
                            adjustTopicSpecials(amount, contributor);
                        }
                        else if (amount === VidDbl) {
                            adjustTopicSpecials(amount, contributor);
                            if (CountrSpec)
                                if (CountrSpec1) {
                                    if (CountrSpec2) {
                                        if (CountrSpec3) {
                                            CountrSpec3 = false;
                                            RoomMsg = "/tip 250  CountrSpecial for " + contributor;
                                            postToRoom(RoomMsg);
                                            RoomMsg = "Just 2 more 250tk contributions remain!!";
                                            postToRoom(RoomMsg);
                                        }
                                        else {
                                            CountrSpec2 = false;
                                            RoomMsg = "/tip 250  CountrSpecial for " + contributor;
                                            postToRoom(RoomMsg);
                                            RoomMsg = "Just 1 more 250tk contribution remains!!";
                                            postToRoom(RoomMsg);
                                        }
                                    }
                                    else {
                                        CountrSpec1 = false;
                                        CountrSpec = false;
                                        RoomMsg = "/tip 250  CountrSpecial for " + contributor;
                                        postToRoom(RoomMsg);
                                        RoomMsg = ":wowo No more 250tk contributions remain :sadkid";
                                        postToRoom(RoomMsg);
                                    }
                                }
                        }
                        else if (amount === Vid1) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else if (amount === Vid2) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else if (amount === Vid3) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else if (amount === Vid4) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else if (amount === Vid5) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else if (amount === Vid6) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else if (amount === Vid7) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else if (amount === Vid8) {
                            adjustVideoSpecials(amount, contributor);
                        }
                        else {
                            adjustCountDown(amount);
                        }
                    }
                    else {
                        RoomMsg = "I'm sorry only TestCounter has authorisation to test this counter's functionality";
                        postToRoom(RoomMsg);
                    }
                }
            }
            else {
                RoomMsg = "Sorry test mode isn't turned on";
                postToRoom(RoomMsg);
            }
        }
        function requestForTopic (QuestionAmount, contributor) {
            ModelMsg = "was " + QuestionAmount + " from " + contributor + " for topic? reply in PM yes or no";
            postToModel(ModelMsg);
            ParkMsg = "was " + QuestionAmount + " from " + contributor + " for topic? reply in PM yes or no";
            postToPark(ParkMsg);
        }
        function HandleRequestForTopic(Req, msgSpan) {
            var Request = Req[1];
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster == "PM from Parkesy86") || (poster == ("PM from " + currentModelName))) {
                    if (Request == "yes") {
                        var amount = QuestionAmount;
                        adjustCountDown(amount);
                        ModelMsg = amount + " has been deducted as requested";
                        postToModel(ModelMsg);
                        ParkMsg = amount + " has been deducted as requested";
                        postToPark(ParkMsg);
                        QuestionAmount = 0;
                    }
                    else if (Request == "no") {
                        QuestionAmount = 0;
                    }
                    else {
                    }
                }
            }
        }

        //game functions

        //Setting & Stopping Games, Tis & Prizes
        function setGameTips(gametip, msgSpan) {
            var tipval = parseInt(gametip[2]);
            var game = gametip[1];
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (game == "war") {
                        WarTip = tipval;
                        RoomMsg = "War tip has been set by " + poster + " to " + tipval;
                        postToRoom(RoomMsg);
                    }
                    else if (game == "blackjack") {
                        BlackJackTip = tipval;
                        RoomMsg = "BlackJack tip has been set by " + poster + " to " + tipval;
                        postToRoom(RoomMsg);
                    }
                    else if (game == "spin") {
                        SpinTip = tipval;
                        RoomMsg = "Spin tip has been set by " + poster + " to " + tipval;
                        postToRoom(RoomMsg);
                    }
                    else {
                        RoomMsg = "I'm sorry but " + game + " isn't listed in my catalogue";
                        postToRoom(RoomMsg);
                    }
                }
            }
        }
        function startGame(gameval, msgSpan) {
            var game = gameval[2];
            var command = gameval[1];
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (game == "blackjack") {
                        if (command == "start") {
                            if (isBlackJackActive) {
                                RoomMsg = "I can't start a new game of BlackJack until this one has been cancelled";
                                postToRoom(RoomMsg);
                            }
                            else if (BlackJackTip > 0) {
                                if (BlackJackPrizeAssigned) {
                                    isBlackJackActive = true;
                                    RoomMsg = "BlackJack game has been started, tip " + BlackJackTip + " to play a hand & win " + BlackJackPrize;
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isBlackJackActive = true;
                                    RoomMsg = "BlackJack game has been started, tip " + BlackJackTip + " to play a hand, no prize is set for winning ";
                                    postToRoom(RoomMsg);
                                }
                            }
                            else {
                                if (BlackJackPrizeAssigned) {
                                    isBlackJackActive = true;
                                    RoomMsg = "BlackJack game has been started, no tip value set to play a hand & win " + BlackJackPrize;
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isBlackJackActive = true;
                                    RoomMsg = "BlackJack game has been started, tip no tip value set to play to play a hand, no prize is set for winning ";
                                    postToRoom(RoomMsg);
                                }
                            }
                        }
                        else if (command == "stop") {
                            isBlackJackActive = false;
                            BlackJackPrize = [];
                            BlackJackPrizeAssigned = false;
                            BlackJackTip = 0;
                            RoomMsg = game + " has been stopped & values cleared";
                            postToRoom(RoomMsg);
                        }
                    }
                    if (game == "guess") {
                        if (command == "start") {
                            if (isGuessActive) {
                                RoomMsg = "I can't start a new game of guess the number until the current one has been cancelled";
                                postToRoom(RoomMsg);
                            }
                            else {
                                if (GuessPrizeAssigned) {
                                    isGuessActive = true;
                                    chance = Math.floor(Math.random() * 100) + 1;
                                    RoomMsg = "Guessing game has been started, tip between 1-100 to guess a number & win " + GuessPrize;
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isGuessActive = true;
                                    chance = Math.floor(Math.random() * 100) + 1;
                                    RoomMsg = "Guessing game has been started, tip between 1-100 to guess a number, no prize has been assigned";
                                    postToRoom(RoomMsg);
                                }
                            }
                        }
                        if (command == "stop") {
                            GuessPrize = [];
                            GuessPrizeAssigned = false;
                            chance = 0;
                            RoomMsg = game + " has been stopped & values cleared";
                            postToRoom(RoomMsg);
                        }
                    }
                    if (game == "war") {
                        if (command == "start") {
                            if (isWarActive) {
                                RoomMsg = "I can't start a new game of war until the current one has been cancelled";
                                postToRoom(RoomMsg);
                            }
                            else if (WarTip > 0) {
                                if (WarPrizeAssigned) {
                                    isWarActive = true;
                                    RoomMsg = "War game has been started, tip " + WarTip + " to play war & win " + WarPrize;
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isWarActive = true;
                                    RoomMsg = "War game has been started, tip " + WarTip + " to play war, there is no prize for winning";
                                    postToRoom(RoomMsg);
                                }
                            }
                            else {
                                if (WarPrizeAssigned) {
                                    isWarActive = true;
                                    RoomMsg = "War game has been started but no tip value stated to play, you can play manually or set WarTip value";
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isWarActive = true;
                                    RoomMsg = "War game has been started but no tip value or prize has been set, you can play manually or set these values";
                                    postToRoom(RoomMsg);
                                }
                            }
                        }
                        else if (command == "stop") {
                            isWarActive = false;
                            WarPrize = [];
                            WarPrizeAssigned = false;
                            WarTip = 0;
                            RoomMsg = game + " has been stopped & values cleared";
                            postToRoom(RoomMsg);
                        }
                    }
                    if (game == "spin") {
                        if (command == "start") {
                            if (isSpinActive) {
                                RoomMsg = "I can't start a new game of Spin The Wheel until this one has been cancelled";
                                postToRoom(RoomMsg);
                            }
                            else if (SpinTip > 0) {
                                if (SpinPrizeAssigned) {
                                    isSpinActive = true;
                                    RoomMsg = "Spin The Wheel has been started, tip " + SpinTip + " to play a hand & win " + SpinPrize;
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isSpinActive = true;
                                    RoomMsg = "Spin The Wheel has been started, tip " + SpinTip + " to play a hand, no prize is set for winning ";
                                    postToRoom(RoomMsg);
                                }
                            }
                            else {
                                if (SpinPrizeAssigned) {
                                    isSpinActive = true;
                                    RoomMsg = "Spin The Wheel has been started, no tip value set to play a hand & win " + SpinPrize;
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    isSpinActive = true;
                                    RoomMsg = "Spin The Wheel has been started, no tip value set to play to play a hand";
                                    postToRoom(RoomMsg);
                                }
                            }
                        }
                        else if (command == "stop") {
                            isSpinActive = false;
                            SpinPrizeAssigned = false;
                            SpinTip = 0;
                            RoomMsg = game + " has been stopped & values cleared";
                            postToRoom(RoomMsg);
                        }
                    }
                    if (game == "special") {
                        if (command == "start") {
                            CountrSpec = true;
                            CountrSpec1 = true;
                            CountrSpec2 = true;
                            CountrSpec3 = true;
                            RoomMsg = "Special contributions have been started";
                            postToRoom(RoomMsg);
                        }
                        else if (command == "stop") {
                            CountrSpec = false;
                            CountrSpec1 = false;
                            CountrSpec2 = false;
                            CountrSpec3 = false;
                            RoomMsg = "Special contribution has been stoopped";
                            postToRoom(RoomMsg);
                        }
                    }
                }
                else if (poster == "AnaliasCountr") {
                    RoomMsg = ":spyou Countr, you should know better!";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = ":mno " + poster + " you are not allowed to " + command + game;
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to " + command + game;
                    postToPark(ParkMsg);
                }
            }
        }
        function setPrizes(gamePrize, msgSpan) {
            var game = gamePrize[1];
            var gameprz = gamePrize[2];
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (game == "war") {
                        WarPrizeAssigned = true;
                        WarPrize = gameprz;
                        RoomMsg = "War prize has been set to " + WarPrize;
                        postToRoom(RoomMsg);
                    }
                    else if (game == "guess") {
                        GuessPrizeAssigned = true;
                        GuessPrize = gameprz;
                        RoomMsg = "Guessing game prize has been set to " + GuessPrize;
                        postToRoom(RoomMsg);
                    }
                    else if (game == "blackjack") {
                        BlackJackPrizeAssigned = true;
                        BlackJackPrize = gameprz;
                        RoomMsg = "Blackjack prize set to " + BlackJackPrize;
                        postToRoom(RoomMsg);
                    }
                    else if (game == "spin") {
                        SpinPrizeAssigned = true;
                        RoomMsg = "Spin prize has been set to " + SpinPrize;
                        postToRoom(RoomMsg);
                    }
                }
                else {
                    RoomMsg = "Sorry, you can not set game prizes";
                    postToRoom(RoomMsg);
                }
            }
        }
        function stopAllGames(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    GuessPrize = 0;
                    GuessPrizeAssigned = false;
                    isGuessActive = false;
                    chance = 0;
                    WarPrize = 0;
                    WarPrizeAssigned = false;
                    isWarActive = false;
                    WarTip = 0;
                    isBlackJackActive = false;
                    isSpinActive = false;
                    BlackJackTip = 0;
                    SpinTip = 0;
                    BlackJackPrize = 0;
                    BlackJackPrizeAssigned = false;
                    isGameInProgress = false;
                    SpinPrizeAssigned = false;
                    BlackJackPlayer = '';
                    NewModelHand = 0;
                    NewPlayerHand = 0;
                    ModelSet = false;
                    PlayerSet = false;
                    PlayerHand = 0;
                    ModelHand = 0;
                    AskedModelOnce = false;
                    RoomMsg = "Success, all games, prizes and amounts have been cancelled";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " you can't stop all the games";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to stop the war game";
                    postToPark(ParkMsg);
                }
            }
        }
        function postGameValues(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (isWarActive) {
                        if (WarPrizeAssigned) {
                            RoomMsg = "war is active to play tip: " + WarTip + "war prize is: " + WarPrize;
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "war is active to play, no prize set for winners, to play tip: " + WarTip;
                            postToRoom(RoomMsg);
                        }
                    }
                    if (isSpinActive){
                        RoomMsg = "Spin The Wheel is active to play, tip: " + SpinTip + " for your chance to win";
                        postToRoom(RoomMsg);
                    }
                    if (isBlackJackActive){
                        if (BlackJackPrizeAssigned){
                            RoomMsg = "BlackJack is active to play, tip: " + BlackJackTip + " prize is " + BlackJackPrize;
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "BlackJack is active to play, no prize set for winners, to play tip: " + BlackJackTip;
                            postToRoom(RoomMsg);
                        }
                    }
                    if (isGuessActive) {
                        if (GuessPrizeAssigned) {
                            RoomMsg = "/pm " + poster + " the random number for today is " + chance + " whoever tips this wins (max 1 person)";
                            postToRoom(RoomMsg);
                            RoomMsg = currentModelName + "'s Random Number Game, simply tip an amount between 1-100 and if it matches the random number you win (max 1 person per game)";
                            postToRoom(RoomMsg);
                            RoomMsg = "prize for guessing the correct number is: " + GuessPrize;
                            postToRoom(RoomMsg);
                            RoomMsg = "numbers guessed so far are: " + chanceGuessed.sort();
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "/pm " + poster + " the random number for today is " + chance + " whoever tips this wins (max 1 person)";
                            postToRoom(RoomMsg);
                            RoomMsg = currentModelName + "'s Random Number Game, simply tip an amount between 1-100 and if it matches the random number you win (max 1 person per game)";
                            postToRoom(RoomMsg);
                            RoomMsg = "no prize has been set for winning the guessing game";
                            postToRoom(RoomMsg);
                            RoomMsg = "numbers guessed so far are: " + chanceGuessed.sort();
                            postToRoom(RoomMsg);
                        }
                    }
                    else {
                        RoomMsg = "no other games are currently running, start games by turning them on with chat commands";
                        postToRoom(RoomMsg);
                    }
                }
                else {
                    if (isWarActive) {
                        if (WarPrizeAssigned) {
                            RoomMsg = "war is active to play tip: " + WarTip + " prize is " + WarPrize;
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "war is active to play, no prize set for winners, to play tip: " + WarTip;
                            postToRoom(RoomMsg);
                        }
                    }
                    if (isGuessActive) {
                        if (GuessPrizeAssigned) {
                            RoomMsg = currentModelName + "'s Random Number Game, simply tip an amount between 1-100 and if it matches the random number you win (max 1 person per game)";
                            postToRoom(RoomMsg);
                            RoomMsg = "prize for guessing the correct number using tip amount is: " + GuessPrize;
                            postToRoom(RoomMsg);
                            RoomMsg = "numbers guessed so far are: " + chanceGuessed.sort();
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = currentModelName + "'s Random Number Game, simply tip an amount between 1-100 and if it matches the random number you win (max 1 person per game)";
                            postToRoom(RoomMsg);
                            RoomMsg = "no prize has been set for winning the guessing game";
                            postToRoom(RoomMsg);
                            RoomMsg = "numbers guessed so far are: " + chanceGuessed.sort();
                            postToRoom(RoomMsg);
                        }
                    }
                    if (isSpinActive){
                        RoomMsg = "Spin The Wheel is active to play, tip: " + SpinTip + " for your chance to win";
                        postToRoom(RoomMsg);
                    }
                    if (isBlackJackActive){
                        if (BlackJackPrizeAssigned){
                            RoomMsg = "BlackJack is active to play, tip: " + BlackJackTip;
                            postToRoom(RoomMsg);
                            RoomMsg = "BlackJack prize is: " + BlackJackPrize;
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "BlackJack is active to play, no prize set for winners, to play tip: " + BlackJackTip;
                            postToRoom(RoomMsg);
                        }
                    }
                    else {
                        RoomMsg = "no other games are currently running, start games by turning them on with chat commands";
                        postToRoom(RoomMsg);
                    }
                }
            }
        }

        //BlackJack Game
        function assignBlackJackPlayer(play) {
            BlackJackPlayer = play;
        }
        function BlackJackInitialHand(play) {
            isGameInProgress = true;
            PlayerSet = false;
            ModelSet = false;
            var PlayerCard1 = Math.floor(Math.random() * 11) + 1;
            var PlayerCard2 = Math.floor(Math.random() * 11) + 1;
            var ModelCard1 = Math.floor(Math.random() * 11) + 1;
            var ModelCard2 = Math.floor(Math.random() * 11) + 1;
            assignBlackJackPlayer(play);
            PlayerHand = PlayerCard1 + PlayerCard2;
            ModelHand = ModelCard1 + ModelCard2;
            RoomMsg = BlackJackPlayer + " you have " + PlayerCard1 + " & " + PlayerCard2;
            postToRoom(RoomMsg);
            RoomMsg = currentModelName + " has " + ModelCard1 + " showing";
            postToRoom(RoomMsg);
            ModelMsg = " your hand is " + ModelHand;
            postToModel(ModelMsg);
            StoredHand(PlayerHand, ModelHand);
            Check1stHand(BlackJackPlayer);
        }
        function BlackJackManualHand(BlkJkMan, msgSpan) {
            var poster;
            PlayerSet = false;
            ModelSet = false;
            var play = BlkJkMan[1];
            assignBlackJackPlayer(play);
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    isGameInProgress = true;
                    var PlayerCard1 = Math.floor(Math.random() * 11) + 1;
                    var PlayerCard2 = Math.floor(Math.random() * 11) + 1;
                    var ModelCard1 = Math.floor(Math.random() * 11) + 1;
                    var ModelCard2 =  Math.floor(Math.random() * 11) + 1;
                    PlayerHand = PlayerCard1 + PlayerCard2;
                    ModelHand = ModelCard1 + ModelCard2;
                    RoomMsg = BlackJackPlayer + " you have " + PlayerCard1 + " & " + PlayerCard2;
                    postToRoom(RoomMsg);
                    RoomMsg = currentModelName + " has " + ModelCard1 + " showing";
                    postToRoom(RoomMsg);
                    postChatMessage("/pm " + currentModelName + " your hand is " + ModelHand);
                    StoredHand(PlayerHand, ModelHand, BlackJackPlayer);
                    Check1stHand();
                }
            }
        }
        function StoredHand(PlayerHand, ModelHand) {
            NewPlayerHand = PlayerHand;
            NewModelHand = ModelHand;
        }
        function Check1stHand(BlackJackPlayer) {
            if (PlayerHand === 21) {
                RoomMsg = BlackJackPlayer + " has BlackJack!";
                postToRoom(RoomMsg);
                PlayerSet = true;
                ModelSet = true;
                CallWinner();
            }
            else if (ModelHand === 21) {
                RoomMsg = currentModelName + " has BlackJack!";
                postToRoom(RoomMsg);
                CallWinner();
            }
            else {
                RoomMsg = BlackJackPlayer + " would you like to draw! a card or stay! / stick!";
                postToRoom(RoomMsg);
            }
        }
        function DrawCard(command, msgSpan) {
            var choice = command[1];
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                var AdditionalCard = Math.floor(Math.random() * 11) + 1;
                if (poster === BlackJackPlayer) {
                    if (choice == "draw!") {
                        if (PlayerSet) {
                            RoomMsg = BlackJackPlayer + " you can't change your mind now";
                            postToRoom(RoomMsg);
                        }
                        else {
                            PlayerHand = NewPlayerHand + AdditionalCard;
                            RoomMsg = BlackJackPlayer + " has a hand of " + PlayerHand;
                            postToRoom(RoomMsg);
                            if (PlayerHand >= 22) {
                                RoomMsg = BlackJackPlayer + " bust";
                                postToRoom(RoomMsg);
                                NewPlayerHand = 0;
                                NewModelHand = 0;
                                PlayerHand = 0;
                                ModelHand = 0;
                                ModelSet = false;
                                PlayerSet = false;
                                isGameInProgress = false;
                            }
                            else if (PlayerHand === 21) {
                                RoomMsg = BlackJackPlayer + " has 21, dealer?";
                                postToRoom(RoomMsg);
                                PlayerSet = true;
                                StoredHand(PlayerHand, ModelHand);
                                CallWinner();
                            }
                            else {
                                RoomMsg = BlackJackPlayer + " would you like to draw or stick?";
                                postToRoom(RoomMsg);
                            }
                        }
                    }
                    else if ((choice == "stay!") || (choice == "stick!")) {
                        PlayerSet = true;
                        RoomMsg = currentModelName + " has a hand of  " + NewModelHand;
                        postToRoom(RoomMsg);
                        if (ModelHand >= 17) {
                            if (ModelHand >= 22) {
                                RoomMsg = currentModelName + " bust";
                                postToRoom(RoomMsg);
                                NewPlayerHand = 0;
                                NewModelHand = 0;
                                PlayerHand = 0;
                                ModelHand = 0;
                                ModelSet = false;
                                PlayerSet = false;
                                isGameInProgress = false;
                            }
                            else {
                                PlayerSet = true;
                                StoredHand(PlayerHand, ModelHand);
                                CallWinner();
                            }
                        }
                        else {
                            RoomMsg = currentModelName + " would you like to draw! or stay!";
                            postToRoom(RoomMsg);
                            StoredHand(PlayerHand, ModelHand);
                        }
                    }
                }
                else if ((poster === currentModelName) || (poster == "Parkesy86") || (poster === helper)) {
                    if (PlayerSet) {
                        if (choice == "draw!") {
                            ModelHand = NewModelHand + AdditionalCard;
                            RoomMsg = currentModelName + " now has a hand of " + ModelHand;
                            StoredHand(ModelHand);
                            if (ModelHand >= 17) {
                                RoomMsg = currentModelName + " has to stick on hands 17 or over";
                                postToRoom(RoomMsg);
                                StoredHand(PlayerHand, ModelHand);
                                ModelSet = true;
                                CallWinner();
                            }
                            else {
                                RoomMsg = currentModelName + " would you like to draw again or stay?";
                                postToRoom(RoomMsg);
                                StoredHand(PlayerHand, ModelHand);
                            }
                        }
                        else if ((choice == "stay!") || (choice == "stick!")) {
                            StoredHand(PlayerHand, ModelHand);
                            ModelSet = true;
                            RoomMsg = currentModelName + " has a hand of  " + NewModelHand;
                            postToRoom(RoomMsg);
                            CallWinner();
                        }
                    }
                    else {
                        RoomMsg = currentModelName + " you cant draw until the player has finished";
                        postToRoom(RoomMsg);
                    }
                }
                else if (poster == "AnaliasCountr") {
                    RoomMsg = '';
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = "Sorry " + poster + " you aren't playing this round";
                    postToRoom(RoomMsg);
                }
            }
        }
        function CallWinner() {
            if ((ModelSet) && (PlayerSet)) {
                isGameInProgress = false;
                if (NewPlayerHand > NewModelHand) {
                    NewPlayerHand = 0;
                    NewModelHand = 0;
                    PlayerHand = 0;
                    ModelHand = 0;
                    ModelSet = false;
                    PlayerSet = false;
                    isGameInProgress = false;
                    AskedModelOnce = false;
                    if (BlackJackPrizeAssigned) {
                        RoomMsg = BlackJackPlayer + " has won " + BlackJackPrize;
                        postToRoom(RoomMsg);
                        BlackJackPlayer = [];
                    }
                    else {
                        RoomMsg = BlackJackPlayer + " has won";
                        postToRoom(RoomMsg);
                        BlackJackPlayer = [];
                    }
                }
                else if (NewModelHand > NewPlayerHand) {
                    RoomMsg = currentModelName + " has won";
                    postToRoom(RoomMsg);
                    NewPlayerHand = 0;
                    NewModelHand = 0;
                    PlayerHand = 0;
                    ModelHand = 0;
                    ModelSet = false;
                    PlayerSet = false;
                    isGameInProgress = false;
                    AskedModelOnce = false;
                    BlackJackPlayer = [];
                }
                else if (NewModelHand === NewPlayerHand) {
                    RoomMsg = "no winner, lets draw again";
                    postToRoom(RoomMsg);
                    NewPlayerHand = 0;
                    NewModelHand = 0;
                    PlayerHand = 0;
                    ModelHand = 0;
                    ModelSet = false;
                    PlayerSet = false;
                    isGameInProgress = false;
                    BlackJackManualHand();
                    AskedModelOnce = false;
                }
                else {
                    RoomMsg = ":mmath I have no idea why I cant decide who is winning with hands " + NewPlayerHand + NewModelHand;
                    postToRoom(RoomMsg);
                }
            }
            else if (NewPlayerHand >= 22) {
                RoomMsg = BlackJackPlayer + " has bust";
                postToRoom(RoomMsg);
                BlackJackPlayer = [];
            }
            else if (NewModelHand >= 22) {
                RoomMsg = currentModelName + " has bust";
                postToRoom(RoomMsg);
                BlackJackPlayer = [];
            }
            else if (PlayerSet) {
                RoomMsg = currentModelName + " would you like to draw/stick/stay";
                postToRoom(RoomMsg);
            }
            else if (ModelSet) {
                RoomMsg = BlackJackPlayer + " would you like to draw/stick/stay";
                postToRoom(RoomMsg);
            }
        }

        //Spin The Wheel Game
        function SpinTheWheel(contributor) {
            if (isSpinActive) {
                var WheelVal1 = ["Free Spin", "Spank", "Flash", "Truth", "Dare", "Ban A Basic", "20% off any video", "Free Picture Gallery", "Free SnapChat For 1 month"];
                var WheelSpin1 = Math.floor(Math.random() * 9);
                var SpinPlayer = contributor;
                var SpinPrize = WheelVal1[WheelSpin1];

                if (SpinPrize == "Spank") {
                    var WheelVal2 = ["Player", "Model"];
                    var WheelSpin2 = Math.floor(Math.random() * 2) +1;
                    var SpinValue = WheelVal2[WheelSpin2];
                    if (SpinValue == "Player") {
                        RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to " + SpinPrize + " themselves";
                        postToRoom(RoomMsg);
                    }
                    else if (SpinValue == "Model") {
                        RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to " + SpinPrize + " themselves";
                        postToRoom(RoomMsg);
                    }
                }
                else if (SpinPrize == "Flash") {
                    var WheelVal3 = ["Player", "Model"];
                    var WheelSpin3 = Math.floor(Math.random() * 2) +1;
                    var Spin2Value = WheelVal3[WheelSpin3];
                    if (Spin2Value == "Player") {
                        RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to " + SpinPrize + " for " + currentModelName;
                        postToRoom(RoomMsg);
                    }
                    else if (Spin2Value == "Model") {
                        RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to " + SpinPrize + " for " + SpinPlayer;
                        postToRoom(RoomMsg);
                    }
                }
                else if (SpinPrize == "Truth") {
                    var WheelVal4 = ["Player", "Model"];
                    var WheelSpin4 = Math.floor(Math.random() * 2) +1;
                    var Spin3Value = WheelVal4[WheelSpin4];
                    if (Spin3Value == "Player") {
                        RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to answer " + SpinPrize + "fully a question from " + currentModelName;
                        postToRoom(RoomMsg);
                    }
                    else if (Spin3Value == "Model") {
                        RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to answer " + SpinPrize + "fully a question from " + SpinPlayer;
                        postToRoom(RoomMsg);
                    }
                }
                else if (SpinPrize == "Dare") {
                    var WheelVal5 = ["Player", "Model"];
                    var WheelSpin5 = Math.floor(Math.random() * 2) +1;
                    var Spin4Value = WheelVal5[WheelSpin5];
                    if (Spin4Value == "Player") {
                        RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to do a " + SpinPrize + " from " + currentModelName;
                        postToRoom(RoomMsg);
                    }
                    else if (Spin4Value == "Model") {
                        RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to do a " + SpinPrize + " from " + SpinPlayer;
                        postToRoom(RoomMsg);
                    }
                }
                else if (SpinPrize == "Free Spin"){
                    RoomMsg = SpinPlayer + "'s spin result is " + SpinPrize;
                    postToRoom(RoomMsg);
                    contributor = SpinPlayer;
                    postCountTimeout = setTimeout(function () {
                        SpinTheWheel(contributor);
                    }, 10000);
                }
                else {
                    RoomMsg = SpinPlayer + "'s spin result is " + SpinPrize;
                    postToRoom(RoomMsg);
                }
            }
            else {
                RoomMsg = "Spin the wheel is not active, I have reset the Spin The Wheel Tip Value back to 0";
                postToRoom(RoomMsg);
                SpinTip = 0;
            }
        }
        function ManualSpinTheWheel(Spin, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    var WheelVal1 = ["Free Spin", "Spank", "Flash", "Truth", "Dare", "Ban A Basic", "20% off any video", "Free Picture Gallery", "Free SnapChat For 1 month"];
                    var WheelSpin1 = Math.floor(Math.random() * 9);
                    var SpinPlayer = Spin[1];
                    var contributor = Spin[1];
                    var SpinPrize = WheelVal1[WheelSpin1];
                    var WheelVal2 = ["Player", "Model"];
                    var WheelSpin2 = Math.floor(Math.random() * 2) +1;
                    var SpinValue = WheelVal2[WheelSpin2];
                    if (SpinPrize == "Spank") {
                        if (SpinValue == "Player") {
                            RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to " + SpinPrize + " themselves";
                            postToRoom(RoomMsg);
                        }
                        else if (SpinValue == "Model") {
                            RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to " + SpinPrize + " themselves";
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (SpinPrize == "Flash") {
                        if (SpinValue == "Player") {
                            RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to " + SpinPrize + " for " + currentModelName;
                            postToRoom(RoomMsg);
                        }
                        else if (SpinValue == "Model") {
                            RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to " + SpinPrize + " for " + SpinPlayer;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (SpinPrize == "Truth") {
                        if (SpinValue == "Player") {
                            RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to answer " + SpinPrize + "fully a question from " + currentModelName;
                            postToRoom(RoomMsg);
                        }
                        else if (SpinValue == "Model") {
                            RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to answer " + SpinPrize + "fully a question from " + SpinPlayer;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (SpinPrize == "Dare") {
                        if (SpinValue == "Player") {
                            RoomMsg = SpinPlayer + "'s spin result is " + SpinPlayer + " has to do a " + SpinPrize + " from " + currentModelName;
                            postToRoom(RoomMsg);
                        }
                        else if (SpinValue == "Model") {
                            RoomMsg = SpinPlayer + "'s spin result is " + currentModelName + " has to do a " + SpinPrize + " from " + SpinPlayer;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (SpinPrize == "Free Spin"){
                        RoomMsg = SpinPlayer + "'s spin result is " + SpinPrize;
                        postToRoom(RoomMsg);
                        isSpinActive = true;
                        postCountTimeout = setTimeout(function () {
                            SpinTheWheel(contributor);
                        }, 5000);
                    }
                    else {
                        RoomMsg = SpinPlayer + "'s spin result is " + SpinPrize;
                        postToRoom(RoomMsg);
                    }
                }
            }
        }

        //War Game
        function StartManualWar(War, msgSpan) {
            WarPlayer = War[1];
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86") || (poster == "HoleeeSheeet") || (poster === helper)) {
                    RoomMsg = "Let's shuffle before drawing :deckshuffle .........";
                    postToRoom(RoomMsg);
                    postCountTimeout = setTimeout(function () {
                        postCountTimeout = null;
                        DrawPlayerCard(WarPlayer);
                    }, 3000);
                    postCountTimeout = setTimeout(function () {
                        postCountTimeout = null;
                        DrawModelCard();
                    }, 5000);
                    postCountTimeout = setTimeout(function () {
                        postCountTimeout = null;
                        CompileWarWinner();
                    }, 7000);
                }
            }
        }
        function StartTipWar(contributor) {
            WarPlayer = contributor;
            if (isWarActive) {
                postCountTimeout = setTimeout(function () {
                    postCountTimeout = null;
                    DrawPlayerCard(WarPlayer);
                }, 3000);
                postCountTimeout = setTimeout(function () {
                    postCountTimeout = null;
                    DrawModelCard();
                }, 5000);
                postCountTimeout = setTimeout(function () {
                    postCountTimeout = null;
                    CompileWarWinner();
                }, 7000);
            }
            else {
                ParkMsg = "War isn't active for tips but I got here somehow!";
                postToPark(ParkMsg);
            }
        }
        function DrawPlayerCard(WarPlayer){
            CardSuits = ["Diamonds", "Clubs", "Hearts", "Spades"];
            PlayerCardLocation = Math.floor(Math.random() *13) +1;
            var PlayerSuitLocation = Math.floor(Math.random() *4);
            var PlayerSuit = CardSuits[PlayerSuitLocation];
            RoomMsg = WarPlayer + " you have drew the " + PlayerCardLocation + " of " + PlayerSuit;
            postToRoom(RoomMsg);
        }
        function DrawModelCard(){
            ModelCardLocation = Math.floor(Math.random() *13) +1;
            var ModelSuitLocation = Math.floor(Math.random() *4);
            var ModelSuit = CardSuits[ModelSuitLocation];
            RoomMsg = currentModelName + " you have drew the " + ModelCardLocation + " of " + ModelSuit;
            postToRoom(RoomMsg);
        }
        function CompileWarWinner() {
            if (PlayerCardLocation > ModelCardLocation){
                if (WarPrizeAssigned){
                    RoomMsg = WarPlayer + " has won the model at War & won " + WarPrize;
                    postToRoom(RoomMsg);
                    ModelMsg = WarPlayer + " has won " + WarPrize + " by winning a game of War";
                    postToModel(ModelMsg);
                }
                else {
                    RoomMsg = WarPlayer + " has won the model at War";
                    postToRoom(RoomMsg);
                }
            }
            else if (ModelCardLocation > PlayerCardLocation) {
                RoomMsg = currentModelName + " wins this round of War!";
                postToRoom(RoomMsg);
            }
            else if (ModelCardLocation === PlayerCardLocation) {
                RoomMsg = ":wowo it's a draw! drawing again....";
                postToRoom(RoomMsg);
                postCountTimeout = setTimeout(function () {
                    postCountTimeout = null;
                    DrawPlayerCard(WarPlayer);
                    DrawModelCard();
                    CompileWarWinner();
                }, 5000);
            }
            else {
                RoomMsg = "I have no clue how I ended here, something went wrong :mmath";
                postToRoom(RoomMsg);
            }
        }

        //Guessing Game
        function PlayGuessGame(amount, contributor) {
            if (amount === chance) {
                if (GuessPrizeAssigned) {
                    RoomMsg = contributor + " guessed the number & wins " + GuessPrize;
                    postToRoom(RoomMsg);
                    ModelMsg = contributor + " has won the guessing game with the value " + amount + " & won " + GuessPrize;
                    postToModel(ModelMsg);
                    chance = 0;
                    chanceGuessed = [];
                    GuessPrizeAssigned = false;
                    isGuessActive = false;
                    ParkMsg = "winner of chance was " + contributor;
                    postToPark(ParkMsg);
                }
                else {
                    RoomMsg = ":uhuh we have a winner " + contributor;
                    postToRoom(RoomMsg);
                    ModelMsg = contributor + " has won the guessing game with the value " + amount;
                    postToModel(ModelMsg);
                    chance = 0;
                    chanceGuessed = [];
                    GuessPrizeAssigned = false;
                    isGuessActive = false;
                    ParkMsg = "winner of chance was " + contributor;
                    postToPark(ParkMsg);
                }
            }
            else {
                if (amount === 1) {
                    amount = "01";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 2) {
                    amount = "02";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 3) {
                    amount = "03";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 4) {
                    amount = "04";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 5) {
                    amount = "05";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 6) {
                    amount = "06";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 7) {
                    amount = "07";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 8) {
                    amount = "08";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
                else if (amount === 9) {
                    amount = "09";
                    if (chanceGuessed.indexOf(amount) == -1) {
                        chanceGuessed.push(amount);
                    }
                }
            }
        }
        function handleGame(addgame, msgSpan){
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    var amount = parseInt(addgame[1]);
                    if (isGuessActive) {
                        if (amount >= 101) {
                            RoomMsg = "I'm sorry, the maximum amount for the game is 100, to change please contact Parkesy";
                            postToRoom(RoomMsg);
                        }
                        else if (amount === chance) {
                            RoomMsg = ":uhuh we have a winner, please start a new game if you would like to run another one";
                            postToRoom(RoomMsg);
                            isGuessActive = false;
                        }
                        else if (amount === 1) {
                            amount = "01";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 2) {
                            amount = "02";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 3) {
                            amount = "03";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 4) {
                            amount = "04";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 5) {
                            amount = "05";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 6) {
                            amount = "06";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 7) {
                            amount = "07";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 8) {
                            amount = "08";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                        else if (amount === 9) {
                            amount = "09";
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                            else {
                                RoomMsg = amount + " has already been guessed";
                                postToRoom(RoomMsg);
                                RoomMsg = "numbers guessed so far are: " + chanceGuessed.sort();
                                postToRoom(RoomMsg);
                            }
                        }
                        else {
                            if (chanceGuessed.indexOf(amount) == -1) {
                                chanceGuessed.push(amount);
                            }
                        }
                    }
                    else {
                        RoomMsg = "I'm sorry " + poster + " the game is not currently running, please start a new one if you wish to run a game";
                        postToRoom(RoomMsg);
                    }
                }
                else {
                    RoomMsg = ":mno I'm sorry " + poster + " guesses need to be the value of a tip";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to play guessing game without tipping";
                    postToPark(ParkMsg);
                }
            }
        }
        function clearGame(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    chanceGuessed = [];
                    chance = Math.floor((Math.random() * 100) + 1);
                    RoomMsg = "game data has been reset by " + poster;
                    postToRoom(RoomMsg);
                    ModelMsg = " new value is " + chance;
                    postToModel(ModelMsg);
                }
                else {
                    RoomMsg = ":nono I'm sorry " + poster + " you can not remove guessing game data";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to clear game data";
                    postToPark(ParkMsg);
                }
            }
        }

        //Controlling topics and variables
        function postTips(msgSpan) {
            var poster;
            //is the message from authorised people?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "Parkesy86") || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (PostTips) {
                        PostTips = false;
                    }
                    else {
                        PostTips = true;
                    }
                }
            }
        }
        function wakeCountr(msgSpan) {
            var poster;
            //is the message from authorised people?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "Parkesy86") || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        if (poster === currentModelName) {
                            RoomMsg = ":bdhi " + currentModelName + " :bdshy";
                            postToRoom(RoomMsg);
                            ModelMsg = "reply t1 4999 or t1 2555 to set countdown amount";
                            postToModel(ModelMsg);
                        }
                        else {
                            RoomMsg = ":yo5 " + poster;
                            postToRoom(RoomMsg);
                        }
                        postChatMessage("/pm " + poster + " commands have now been enabled for you, you can now control me using PM commands");
                    }
                    else {
                        postChatMessage("/pm " + poster + " I'm sure I have already said hi ")
                    }
                }
                else if (poster == "AnaliasCountr") {}
                else if (poster == "TheyCallMeNig") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":ohi Nig";
                        postToRoom(RoomMsg);
                    }
                    else {}
                }
                else if (poster == "analias_lole") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":sup Steven";
                        postToRoom(RoomMsg);
                    }
                    else {}
                }
                else if (poster == "AnaliasPrince") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":myo Prince";
                        postToRoom(RoomMsg);
                    }
                    else {}
                }
                else if (poster == "AnaliasWes") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":yo7 Wes";
                        postToRoom(RoomMsg);
                    }
                    else {}
                }
                else if (poster == "AnaliasPony") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":wave3 Jony";
                        postToRoom(RoomMsg);
                    }
                    else {}
                }
                else if (poster == "AnaliasPhil") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":wave2 Phil";
                        postToRoom(RoomMsg);
                    }
                    else {}
                }
                else if (poster == "DEnnis") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":yo5 Dennis";
                        postToRoom(RoomMsg);
                    }
                    else {}
                }
                else if (poster == "theseller") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":wazzup Pervy Seller";
                        postToRoom(RoomMsg);
                    }
                    else {}                }
                else if (poster == "Rock_nThatAss") {}
                else if (poster == "AnasFundlet") {}
                else if (poster == "AnaliaOMG") {}
                else if (poster == "AnaliasAss") {}
                else if (poster == "MokkoPoko") {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":yo1 Mokko";
                        postToRoom(RoomMsg);
                    }
                    else {
                    }
                }
                else {
                    if (SaidHi.indexOf(poster) == -1) {
                        SaidHi.push(poster);
                        RoomMsg = ":ohi " + poster;
                        postToRoom(RoomMsg);
                    }
                    else {
                    }
                }
            }
        }
        function toggleTopicVar(TVar, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                var TopicMaster = parseInt(TVar[1]);
                if ((poster === currentModelName) || (poster == "Parkesy86")  || (poster == "PM from Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper) || (poster == ("PM from " + currentModelName)) || (poster == "PM from HoleeeSheeet")) {
                    if (TopicMaster === 4999){
                        Topic1 = 4999;
                        Topic2 = 1999;
                        Topic3 = 1999;
                        Topic4 = 1999;
                        Topic5 = 0;
                        Show1 = " Nakkey ";
                        countDown = Topic1;
                        IsFridayActive = true;
                        autoPostActive = true;
                        showCurrentCountDown();
                        aAutoPost.setAttribute('class', 'show');
                        aPostCurrentCount.setAttribute('class', 'show');
                        postCountTimeout = setTimeout(function () {
                            postCountTimeout = null;
                            ModelMsg = "I have set the topics, just reply: start topic 1  and I will start the countdown :D";
                            postToModel(ModelMsg);
                        }, 5000);
                    }
                    else if (TopicMaster === 2555){
                        Topic1 = 2555;
                        Topic2 = 2555;
                        Topic3 = 1999;
                        Topic4 = 1999;
                        Topic5 = 1999;
                        countDown = Topic1;
                        IsFridayActive = false;
                        autoPostActive = true;
                        showCurrentCountDown();
                        aAutoPost.setAttribute('class', 'show');
                        aPostCurrentCount.setAttribute('class', 'show');
                        postCountTimeout = setTimeout(function () {
                            postCountTimeout = null;
                            ModelMsg = "I have set the topics, just reply: start topic 1  and I will start the countdown :D";
                            postToModel(ModelMsg);
                        }, 5000);
                    }
                    else {
                        Topic1 = TopicMaster;
                        Topic2 = TopicMaster;
                        Topic3 = 1999;
                        Topic4 = 1999;
                        Topic5 = 1999;
                        countDown = Topic1;
                        Show1 = "TBC";
                        AlternateCount = true;
                        autoPostActive = true;
                        showCurrentCountDown();
                        IsFridayActive = false;
                        aAutoPost.setAttribute('class', 'show');
                        aPostCurrentCount.setAttribute('class', 'show');
                        postCountTimeout = setTimeout(function () {
                            postCountTimeout = null;
                            ModelMsg = "I don't have that topic stored but have started that count anyway, just reply: start topic 1  and I will start the countdown :D";
                            postToModel(ModelMsg);
                        }, 5000);
                    }
                }
                else if ((poster == "PM from AnaliasCountr") || (poster == "AnaliasCountr")){}
                else {
                    RoomMsg = "I'm sorry " + poster + " you cant set topics values";
                    postToRoom(RoomMsg);
                }
            }
        }
        function assignShows(show, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper) || (poster == ("PM from " + currentModelName)) || (poster == "PM from Parkesy86") || (poster == "PM from HoleeeSheeet")) {
                    var SType = show[1];
                    if (IsTopic1Active) {
                        Show1 = SType;
                    }
                    else if (IsTopic2Active) {
                        if (IsFridayActive) {
                            Show3 = SType;
                        }
                        else {
                            Show2 = SType;
                        }
                    }
                    else if (IsTopic3Active) {
                        if (IsFridayActive) {
                            Show4 = SType;
                        }
                        else {
                            Show3 = SType;
                        }
                    }
                    else if (IsTopic4Active) {
                        if (IsFridayActive) {
                            Show5 = SType;
                        }
                        else {
                            Show4 = SType;
                        }
                    }
                    else if (IsTopic5Active) {
                        if (IsFridayActive) {}
                        else {
                            Show5 = SType;
                        }
                    }
                    else {
                    }
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " you can not set show types";
                    postToRoom(RoomMsg);
                }
            }
        }
        function toggleSpecials(SVar, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper) || (poster == "PM from " + currentModelName) || (poster == "PM from Parkesy86") || (poster == "PM from HoleeeSheeet")) {
                    var control = SVar[1];
                    var ctrlval = parseInt(SVar[2]);
                    var ctrlvaltxt = SVar[2];
                    if (control == "spank"){
                        SpankSpec = ctrlval;
                        if (SpankSpec > 0) {
                            RoomMsg = "Spank value has been set to " + SpankSpec;
                            postToRoom(RoomMsg);
                        }
                        else {
                            SpankSpec = 99;
                            RoomMsg = "Spank value set to default value of " + SpankSpec;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (control == "flash") {
                        FlashSpec = ctrlval;
                        if (FlashSpec > 0) {
                            RoomMsg = "Flash value has been set to " + FlashSpec;
                            postToRoom(RoomMsg);
                        }
                        else {
                            FlashSpec = 100;
                            RoomMsg = "Flash value has been set to default " + FlashSpec;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (control == "snap") {
                        SnapSpec = ctrlval;
                        if (SnapSpec > 0) {
                            RoomMsg = "SnapChat value has been set to " + SnapSpec;
                            postToRoom(RoomMsg);
                        }
                        else {
                            SnapSpec = 1717;
                            RoomMsg = "SnapChat value has been set to default " + SnapSpec;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (control == "vidsgl") {
                        VidSgl = ctrlval;
                        if (VidSgl > 0) {
                            RoomMsg = "Single Vid value has been set to " + VidSgl;
                            postToRoom(RoomMsg);
                        }
                        else {
                            VidSgl = 500;
                            RoomMsg = "Single Vid value has been set to default " + VidSgl;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (control == "viddbl") {
                        VidDbl = ctrlval;
                        if (VidDbl > 0) {
                            RoomMsg = "Multi Vid value has been set to " + VidDbl;
                            postToRoom(RoomMsg);
                        }
                        else {
                            VidDbl = 999;
                            RoomMsg = "Multi Vid value has been set to default " + VidDbl;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (control == "vidtrp") {
                        VidTrp = ctrlval;
                        if (VidDbl > 0){
                            RoomMsg = "Non topic Vid value has been set to " + VidTrp;
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "Non topic Vid value has been set to default " + VidTrp;
                            postToRoom(RoomMsg);
                        }
                    }
                    else if (control == "vidamt") {
                        VidAmt = ctrlval;
                        RoomMsg = "vid qty in topic has been set to " + VidAmt;
                        postToRoom(RoomMsg);
                    }
                    else if (control == "varvid") {
                        VarVid = ctrlvaltxt;
                        RoomMsg = "video type set to " + VarVid;
                        postToRoom(RoomMsg);

                    }
                    else {
                        RoomMsg = "I'm sorry " + poster + " but " + control + " is not a recognised command prompt";
                        postToRoom(RoomMsg);
                    }
                }
                else {
                    RoomMsg = "Sorry you don't have relevant authorisation to change topic specials";
                    postToRoom(RoomMsg);
                }
            }
        }
        function postLastTopic(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter") || (poster == "Parkesy86") || (poster == "HoleeeSheeet") || (poster === helper)) {
                    if (isCountDownActive) {
                        postCurrentTopic();
                    }
                }
                else if (poster == "AnaliasCountr") {
                    RoomMsg = ":mno countr, you know better!!";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = ":wowo " + poster + " you have a topic?, what was it? :mpervy";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to request topic";
                    postToPark(ParkMsg);
                }
            }
        }
        function stopCount(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === ("PM from " + currentModelName)) || (poster == "PM from TestCounter")  || (poster == "PM from Parkesy86")  || (poster == "PM from HoleeeSheeet") || (poster === helper)) {
                    isCountDownActive = false;
                    autoPostActive = false;
                    countDown = 0;
                    postCurrentCount();
                    showCurrentCountDown();
                    ModelMsg = "count has been stopped and cleared";
                    postToModel(ModelMsg);
                }
                else {
                }
            }
        }
        function startVarCount(start, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "PM from " + currentModelName) || (poster == "Parkesy86") || (poster == "PM from Parkesy86") || (poster == "PM from HoleeeSheeet")) {
                    var topicNumb = parseInt(start[1]);
                    if (topicNumb === 1) {
                        countDown = Topic1;
                        showCurrentCountDown();
                        isCountDownActive = true;
                        IsTopic1Active = true;
                        autoPostActive = true;
                        postCurrentTopic();
                        postChatMessage("make sure to follow " + currentModelName + " on Twitter :doatwit");
                        RoomMsg = ":doa15 all videos available on http://profiles.myfreecams.com/" + currentModelName;
                        postToRoom(RoomMsg);
                        ModelMsg = "Countdown has been started for " + countDown;
                        postToModel(ModelMsg);
                    }
                    else if (topicNumb === 2) {
                        countDown = Topic2;
                        showCurrentCountDown();
                        isCountDownActive = true;
                        IsTopic2Active = true;
                        autoPostActive = true;
                        postCurrentTopic();
                        postChatMessage("make sure to follow " + currentModelName + " on Twitter :doatwit");
                        RoomMsg = ":doa15 all videos available on http://profiles.myfreecams.com/" + currentModelName;
                        postToRoom(RoomMsg);
                        ModelMsg = "Countdown has been started for " + countDown;
                        postToModel(ModelMsg);
                    }
                    else if (topicNumb === 3) {
                        countDown = Topic3;
                        showCurrentCountDown();
                        isCountDownActive = true;
                        IsTopic3Active = true;
                        autoPostActive = true;
                        postCurrentTopic();
                        postChatMessage("make sure to follow " + currentModelName + " on Twitter :doatwit");
                        RoomMsg = ":doa15 all videos available on http://profiles.myfreecams.com/" + currentModelName;
                        postToRoom(RoomMsg);
                        ModelMsg = "Countdown has been started for " + countDown;
                        postToModel(ModelMsg);
                    }
                    else if (topicNumb === 4) {
                        countDown = Topic4;
                        showCurrentCountDown();
                        isCountDownActive = true;
                        IsTopic4Active = true;
                        autoPostActive = true;
                        postCurrentTopic();
                        postChatMessage("make sure to follow " + currentModelName + " on Twitter :doatwit");
                        RoomMsg = ":doa15 all videos available on http://profiles.myfreecams.com/" + currentModelName;
                        postToRoom(RoomMsg);
                        ModelMsg = "Countdown has been started for " + countDown;
                        postToModel(ModelMsg);
                    }
                    else if (topicNumb === 5) {
                        countDown = Topic5;
                        showCurrentCountDown();
                        isCountDownActive = true;
                        IsTopic5Active = true;
                        autoPostActive = true;
                        postCurrentTopic();
                        postChatMessage("make sure to follow " + currentModelName + " on Twitter :doatwit");
                        RoomMsg = ":doa15 all videos available on http://profiles.myfreecams.com/" + currentModelName;
                        postToRoom(RoomMsg);
                        ModelMsg = "Countdown has been started for " + countDown;
                        postToModel(ModelMsg);
                    }
                }
                else if (poster == "AnaliasCountr") {
                    RoomMsg = "";
                    postToRoom(RoomMsg);
                }
                else if (poster == ("PM to " + currentModelName)) {
                    RoomMsg = "";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " you are not allowed to start topics";
                    postToRoom(RoomMsg);
                }
            }
        }
        function StopTopic(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86") || (poster === helper)  || (poster == "HoleeeSheeet")) {
                    if (IsTopic1Active) {
                        countDown = Topic2;
                        IsTopic1Active = false;
                        IsTopic2Active = true;
                        postCurrentCount();
                        postCurrentTopic();
                    }
                    else if (IsTopic2Active) {
                        IsTopic2Active = false;
                        IsTopic3Active = true;
                        countDown = Topic3;
                        postCurrentCount();
                        postCurrentTopic();
                    }
                    else if (IsTopic3Active) {
                        IsTopic3Active = false;
                        IsTopic4Active = true;
                        countDown = Topic4;
                        postCurrentCount();
                        postCurrentTopic();
                    }
                    else if (IsTopic4Active) {
                        IsTopic4Active = false;
                        IsTopic5Active = true;
                        countDown = Topic5;
                        postCurrentCount();
                        postCurrentTopic();
                    }
                    else if (IsTopic5Active) {
                        IsTopic5Active = false;
                        countDown = 0;
                        isCountDownActive = false;
                        autoPostActive = false;
                        postCurrentTopic();
                    }
                }
            }
        }
        function noMoreTopics(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "PM from TestCounter") || (poster == "PM from Parkesy86")) {
                    postChatMessage("Hi Guys! You Rock!, " + SpankSpec + " Spank, " + SnapSpec + "tk SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                    countDown = 0;
                    isCountDownActive = false;
                    autoPostActive = false;
                }
            }
        }
        function TopicAmend(TopicUpdate, msgSpan) {
            //is the message from someone?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {}
            else {
                if (isCountDownActive) {
                    var NewCountDown = parseInt(TopicUpdate[1]);
                    if (NewCountDown >= (countDown + 1)) {
                        ModelMsg = "I have updated the countdown from " + countDown + " to " + NewCountDown;
                        postToModel(ModelMsg);
                        countDown = NewCountDown;
                        postCurrentCount();
                        ParkMsg = "countr was wrong and has been updated";
                        postToPark(ParkMsg);
                    }
                    else if (NewCountDown <= (countDown - 1)) {
                        ModelMsg = "I have updated the countdown from " + countDown + " to " + NewCountDown;
                        postToModel(ModelMsg);
                        countDown = NewCountDown;
                        postCurrentCount();
                        ParkMsg = "countr was wrong and has been updated";
                        postToPark(ParkMsg);
                    }
                    else {
                        ParkMsg = "countr is correct";
                        postToPark(ParkMsg);
                        postChatMessage("make sure to follow " + currentModelName + " on Twitter :doatwit");
                        RoomMsg = ":doa15 all videos available on http://profiles.myfreecams.com/" + currentModelName;
                        postToRoom(RoomMsg);
                    }
                }
            }
        }
        function AdjustValues(Adjust, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter") || (poster == ("PM from " + currentModelName)) || (poster == "Parkesy86") || (poster == "PM from Parkesy86") || (poster == "PM from HoleeeSheeet")) {
                    if (isCountDownActive) {
                        var NewCountDown = parseInt(Adjust[1]);
                        ModelMsg = "Count adjusted from " + countDown + " to " + NewCountDown;
                        postToModel(ModelMsg);
                        countDown = NewCountDown;
                        showCurrentCountDown();
                    }
                }
            }
        }

        //Requesting info from countr
        function assignHelper(helpAsgn, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                var control = helpAsgn[1];
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86") ) {
                    if (isHelperAssigned) {
                        RoomMsg = "helper is already assigned to " + helper + " please remove as helper before reassigning";
                        postToRoom(RoomMsg);
                    }
                    else {
                        if (control === banned) {
                            RoomMsg = control + " is banned from being helper, please choose someone else or contact Park to reset";
                            postToRoom(RoomMsg);
                            RoomMsg = "/pm " + control + " I'm sorry but you are banned for requesting information you shouldn't";
                            postToRoom(RoomMsg);
                            ParkMsg = control + " tried to assign " + banned + " as helper but they are banned";
                            postToPark(ParkMsg);
                        }
                        else {
                            helper = control;
                            RoomMsg = helper + " has been successfully assigned as helper and can now control :countr";
                            postToRoom(RoomMsg);
                            isHelperAssigned = true;
                            helpers.push(helper);
                            ParkMsg = "helpers assigned so far today have been " + helpers;
                            postToPark(ParkMsg);
                        }
                    }
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " but you can not assign helpers";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to assign helper as " + control;
                    postToPark(ParkMsg);
                }
            }
        }
        function postHighTip(msgSpan){
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet")) {
                    RoomMsg = "/pm " + poster + " tip info as requested " + spanHighestTip.innerHTML;
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " has requested highest tip information " + spanHighestTip.innerHTML;
                    postToPark(ParkMsg);
                }
                else if (poster === helper) {
                    RoomMsg = "helper removed from countr";
                    postToRoom(RoomMsg);
                    RoomMsg = "/pm " + helper + " this request is beyond your capabilities, the operator has been notified!";
                    postToRoom(RoomMsg);
                    ModelMsg = "helper tried to request information they shouldn't, as such has been removed as helper and banned for 24hrs";
                    postToModel(ModelMsg);
                    isHelperAssigned = false;
                    if (banned.indexOf(helper) == -1) {
                        banned.push(helper);}
                    ParkMsg = helper + " tried to request highest tip information";
                    postToPark(ParkMsg);
                }
                else {
                    RoomMsg = poster + " I'm sorry only the model can request this information";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to add to request highest tipper information ";
                    postToPark(ParkMsg);
                }
            }
        }
        function postTopic(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            poster = nameSpan.innerHTML.replace(/:/, '');
            if ((poster === currentModelName) || (poster == "TestCounter") || (poster == "Parkesy86") || (poster === helper) || (poster == "HoleeeSheeet")) {
                if (isCountDownActive) {
                    postCurrentTopic();
                }
                else {
                    ModelMsg = "No count is running";
                    postToModel(ModelMsg);
                }
            }
        }
        function postCurrentTopic() {
            if (isCountDownActive) {
                if (IsFridayActive) {
                    if (IsTopic1Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show1 + ", " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else if (IsTopic2Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show3 + ", " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else if (IsTopic3Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show4 + ", " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else if (IsTopic4Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show5 + ", " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm prfl) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else {
                        postChatMessage("Hi Guys! You Rock!, " + SpankSpec + " Spank, " + SnapSpec + "tk SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                }
                else if (AlternateCount) {
                    postChatMessage("Hi Guys! " + countDown + "tk " + Show1 + ", " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                    postVidSpec();
                    ModelMsg = "To change the 'show' just reply with what you would like to change like: change Dress Off  and I will update";
                    postToModel(ModelMsg);
                }
                else {
                    if (IsTopic1Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show1 + ", " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else if (IsTopic2Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show2 + " , " + SpankSpec + " Spank/" + FlashSpec + " Flash, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else if (IsTopic3Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show3 + ", " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else if (IsTopic4Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show4 + ", " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else if (IsTopic5Active) {
                        postChatMessage("Hi Guys! " + countDown + "tk " + Show5 + ", " + SpankSpec + " Spank, " + SnapSpec + " SnapChat 4ever, 2 Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                    else {
                        postChatMessage("Hi Guys! You Rock!, " + SpankSpec + " Spank, " + SnapSpec + "tk SnapChat 4ever, " + VidAmt + " Vids " + VidDbl + "(any frm profile) 5000 for all vids, " + VarVid + " " + VidSgl + ", TY Guys");
                        postVidSpec();
                    }
                }
            }
        }
        function checkHelper(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet")) {
                    if (isHelperAssigned) {
                        RoomMsg = "helper is currently designated to " + helper;
                        postToRoom(RoomMsg);
                    }
                    else {
                        RoomMsg = "Helper is not currently assigned";
                        postToRoom(RoomMsg);
                    }
                }
                else if (poster === helper) {
                    RoomMsg = helper + " you are currently designated as helper";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " you are unable to see who the helper is";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to see who was room helper ";
                    postToPark(ParkMsg);
                }
            }
        }
        function postCountrInfo(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    RoomMsg = "/pm " + poster + " currently running countr which covers topics " + Topic1 + ", " + Topic2 + ", " + Topic3 + ", " + Topic4 + ", " + Topic5;
                    postToRoom(RoomMsg);
                    RoomMsg = "/pm " + poster + " countr also includes the following games: " + gamearray.sort() + " if you want help type 'countr! help'";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " you can not request countr information";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to request countr information";
                    postToPark(ParkMsg);
                }
            }
        }
        function postHelp(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")  || (poster == "HoleeeSheeet") || (poster === helper)) {
                    RoomMsg = "/pm " + poster + " for a full list of countr commands visit: http://www.dropbox.com/s/5bpw6kexul7q7zt/AnaliasCountr.docx?dl=0";
                    postToRoom(RoomMsg);
                    RoomMsg = ":countrhelp";
                    postToRoom(RoomMsg);
                }
                else if (poster == "AnaliasCountr"){
                    RoomMsg = "";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " I can't help you :giggle3";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to get countr help";
                    postToPark(ParkMsg);
                }
            }
        }
        function TipModel(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if (poster == "PM from Parkesy86") {
                    postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1");
                    postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1");
                    postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1"); postChatMessage("/tip 1");
                }
            }
        }
        function postVidSpec() {
            if (CountrSpec) {
                if (CountrSpec1){
                    if (CountrSpec2){
                        if(CountrSpec3){
                            RoomMsg = "The next 3 people to buy the video special from topic gets 250tk towards another!!";
                            postToRoom(RoomMsg);
                            RoomMsg = "Just 3 more 250tk contributions remain!!";
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "The next 2 people to buy the video special from topic gets 250tk towards another!!";
                            postToRoom(RoomMsg);
                            RoomMsg = "Just 2 more 250tk contributions remains!!";
                            postToRoom(RoomMsg);
                        }
                    }
                    else {
                        RoomMsg = "The next person to buy the video special from topic gets 250tk towards another!!";
                        postToRoom(RoomMsg);
                        RoomMsg = "Just 1 more 250tk contribution remains!!";
                        postToRoom(RoomMsg);
                    }
                }
            }
            else {}
        }
        function postSpecial(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster == "Parkesy86") || (poster == "TestCounter")  || (poster === currentModelName) || (poster == "AnaliasCountr")) {
                    if (CountrSpec) {
                        if (CountrSpec1) {
                            if (CountrSpec2) {
                                if (CountrSpec3) {
                                    RoomMsg = "The next 3 people to buy the video special from topic gets 250tk towards another!!";
                                    postToRoom(RoomMsg);
                                }
                                else {
                                    RoomMsg = "The next 2 people to buy the video special from topic gets 250tk towards another!!";
                                    postToRoom(RoomMsg);
                                }
                            }
                            else {
                                RoomMsg = "The next person to buy the video special from topic gets 250tk towards another!!";
                                postToRoom(RoomMsg);
                            }
                        }
                    }
                    else {
                    }
                }
            }
        }
        function ChangePosting(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter") || (poster == "Parkesy86") || (poster == "TestCounter")) {
                    if (PostActive) {
                        ParkMsg = "I wont post anymore to the room";
                        postToPark(ParkMsg);
                        postChatMessage("/pm " + poster + " I won't post anymore to the room");
                        PostActive = false;
                    }
                    else {
                        PostActive = true;
                        ParkMsg = "I will resume posting to the room";
                        postToPark(ParkMsg);
                        postChatMessage("/pm " + poster + " I will resume posting to the room");
                    }
                }
            }
        }

        //General countr operations
        function postCurrentCount() {
            //if there is a cooldown active, queue a post and leave
            if (null !== postCountTimeout) {
                postCountQueued = true;
                return;
            }
            //post the count to chat
                ModelMsg = countDown;
                postToModel(ModelMsg);
            postCountQueued = false;
            if (!autoPostActive)
                aPostCurrentCount.setAttribute('class', 'hidden');
            //start a cooldown
            postCountTimeout = setTimeout(function () {
                postCountTimeout = null;
                if (postCountQueued && isCountDownActive)
                    postCurrentCount();
                if (!autoPostActive)
                    aPostCurrentCount.setAttribute('class', 'show');
            }, 5000);
        }
        function toggleAutoPost() {
            if (!autoPostActive) {
                setElementText(aAutoPost, 'Disable AutoPost');
                //aPostCurrentCount.setAttribute('class', 'hidden');
                autoPostActive = true;
            }
            else {
                setElementText(aAutoPost, 'Enable AutoPost');
                //aPostCurrentCount.setAttribute('class', 'show');
                autoPostActive = false;
            }
        }
        function postChatMessage(msg) {
            if (PostActive) {
                if (killPosting)
                    return;

                txtInput.value = msg;
                btnSend.click();
            }
            else {}
        }
        function showCurrentCountDown() {
            //maAlert("Counter: " + countDown + " tokens left on countdown.");
            setElementText(spanMenuCount, " (" + countDown + ")");
        }
        function maAlert(text) {
            clearTimeout(alertTimeout);
            setElementText(divAlert, text);
            showElement(divAlert);
            alertTimeout = setTimeout(function () {
                hideElement(divAlert);
            }, 5000);
        }
        function parseChatMessage() {
            parseQueue = parseQueue.concat(Array.prototype.slice.call(document.querySelectorAll("#chat_box span.chat"), 0));

            if (parsing)
                return;

            parsing = true;
            while (parseQueue.length > 0) {
                //get the last message
                var msg = parseQueue.pop();
                //if this has already been processed, leave
                if ("true" === msg.getAttribute('ma-processed'))
                    continue;
                //change posting?
                var changing = msg.innerHTML.match(ChangePostingRegex);
                if (changing)
                    ChangePosting(msg);
                //are tips showing?
                var tipPost = msg.innerHTML.match(postTipRegex);
                if (tipPost)
                    postTips(msg);
                //Adjust Count?
                var Adjust = msg.innerHTML.match(AdjustCountRegex);
                if (Adjust)
                    AdjustValues(Adjust, msg);
                //Requestfortopic?
                var Req = msg.innerHTML.match(RequestRegex);
                if (Req)
                    HandleRequestForTopic(Req, msg);
                //Countr Regex?
                var Wake = msg.innerHTML.match(CountrRegex);
                if (Wake)
                    wakeCountr(msg);
                //Topic Update?
                var TopicUpdate = msg.innerHTML.match(TopicUpdateRegex);
                if (TopicUpdate)
                    TopicAmend(TopicUpdate, msg);
                //no more topics?
                var noMore = msg.innerHTML.match(noTopicRegex);
                if (noMore)
                    noMoreTopics(msg);
                //PostTopSpec?
                var comfrom = msg.innerHTML.match(PostTopSpec);
                if (comfrom)
                    postSpecial(msg);
                // test mode command?
                var TestModeMsg = msg.innerHTML.match(TestModeRegex);
                if (TestModeMsg)
                    InitialiseTestMode(TestModeMsg, msg);
                // test tip?
                var spoof = msg.innerHTML.match(spoofTipRegex);
                if (spoof)
                    handleTestTip(spoof, msg);
                // startregex?
                var start = msg.innerHTML.match(startRegex);
                if (start)
                    startVarCount(start, msg);
                //ManualSpin
                var Spin = msg.innerHTML.match(SpinRegex);
                if (Spin)
                    ManualSpinTheWheel(Spin, msg);
                //SpecialsRegex?
                var SVar = msg.innerHTML.match(SpecialRegex);
                if (SVar)
                    toggleSpecials(SVar, msg);
                //topicValRegex?
                var TVar = msg.innerHTML.match(topicValRegex);
                if (TVar)
                    toggleTopicVar(TVar, msg);
                //1st Hand Regex?
                var BlkJkMan = msg.innerHTML.match(BlkJk1Regex);
                if (BlkJkMan)
                    BlackJackManualHand(BlkJkMan, msg);
                //Alternate Hand?
                var command = msg.innerHTML.match(BlkJk2Regex);
                if (command)
                    DrawCard(command, msg);
                //StartGame?
                var gameval = msg.innerHTML.match(startGameRegex);
                if (gameval)
                    startGame(gameval, msg);
                //set game tips
                var gametip = msg.innerHTML.match(setGameTipRegex);
                if (gametip)
                    setGameTips(gametip, msg);
                //set game prizes
                var gamePrize = msg.innerHTML.match(setGamePrizeRegex);
                if (gamePrize)
                    setPrizes(gamePrize, msg);
                //assignHelpRegex?
                var helpAsgn = msg.innerHTML.match(assignHelperRegex);
                if (helpAsgn)
                    assignHelper(helpAsgn, msg);
                //ShowRegex?
                var show = msg.innerHTML.match(ShowRegex);
                if (show)
                    assignShows(show, msg);
                //StopTopicRegex??
                var stptop = msg.innerHTML.match(StopTopicRegex);
                if (stptop)
                    StopTopic(msg);
                //TipHerRegex?
                var tipcmd = msg.innerHTML.match(TipHerRegex);
                if (tipcmd)
                    TipModel(msg);
                //countrRegex?
                var countrversion = msg.innerHTML.match(countrRegex);
                if (countrversion)
                    postCountrInfo(msg);
                //addgameRegex?
                var addgame = msg.innerHTML.match(addgameRegex);
                if (addgame)
                    handleGame(addgame, msg);
                //spamRegex
                var spam = msg.innerHTML.match(spamRegex);
                if (spam)
                    removeSpam(msg);
                //toggleGuessRegex?
                var newgame = msg.innerHTML.match(toggleGuessRegex);
                if (newgame)
                    clearGame(msg);
                //post regex?
                var post = msg.innerHTML.match(postRegex);
                if (post)
                    postTopic(msg);
                //help regex?
                var helpme = msg.innerHTML.match(helpRegex);
                if (helpme)
                    postHelp(msg);
                //stop regex?
                var stp = msg.innerHTML.match(stopRegex);
                if (stp)
                    stopCount(msg);
                //game Regex?
                var game = msg.innerHTML.match(gameRegex);
                if (game)
                    postGameValues(msg);
                //getlastRegex
                var last = msg.innerHTML.match(getlastRegex);
                if (last)
                    postLastTopic(msg);
                //is this a tip?
                var tip = msg.innerHTML.match(tipRegex);
                if (tip)
                    handleTip(tip, msg);
                //playWarRegex?
                var War = msg.innerHTML.match(playWarRegex);
                if (War)
                    StartManualWar(msg, War);
                //removeHelperRegex?
                var helpRmv = msg.innerHTML.match(removeHelperRegex);
                if (helpRmv)
                    removeHelper(helpRmv, msg);
                //checkHelperRegex?
                var chkHelp = msg.innerHTML.match(checkHelperRegex);
                if (chkHelp)
                    checkHelper(msg);
                //stopGamesRegex?
                var stpGames = msg.innerHTML.match(stopGamesRegex);
                if (stpGames)
                    stopAllGames(msg);
                //highTipRegex?
                var highTip = msg.innerHTML.match(highTipRegex);
                if (highTip)
                    postHighTip(msg);
                //addHighRegex?
                var addHighest = msg.innerHTML.match(addHighRegex);
                if (addHighest)
                    addHighTip(addHighest, msg);
                //bannedRemoveRegex?
                var banner = msg.innerHTML.match(bannedRemoveRegex);
                if (banner)
                    removeBanned(banner, msg);
                //addBannedRegex?
                var bana = msg.innerHTML.match(addBannedRegex);
                if (bana)
                    addBanned(bana, msg);
                //toggleNoRegex?
                var nah = msg.innerHTML.match(toggleNoRegex);
                if (nah)
                    toggleNo(nah, msg);
                //nopeRegex?
                var nada = msg.innerHTML.match(nopeRegex);
                if (nada)
                    postNope(msg);
                //PvtRegex
                var PvtSpam = msg.innerHTML.match(PvtRegex);
                if (PvtSpam)
                    RemoveRubbish(msg);
                //check for kill switch msg
                var killMsg = msg.innerHTML.match(killRegex);
                if (killMsg)
                    handleKill(msg);

                //indicate we've processed this msg
                msg.setAttribute("ma-processed", "true");
            }
            parsing = false;
        }
        function toggleNo(nah, msgSpan) {
            var cmdr = nah[1];
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86") || (poster === helper)  || (poster == "HoleeeSheeet")) {
                    if (cmdr == "on") {
                        if (postNo) {
                            RoomMsg = "I am already set to post :doano";
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = "I will watch for them :mlooking";
                            postToRoom(RoomMsg);
                            postNo = true;
                        }
                    }
                    else if (cmdr == "off") {
                        if (postNo) {
                            RoomMsg = "I'm not looking :covereyess";
                            postToRoom(RoomMsg);
                            postNo = false;
                        }
                        else {
                            RoomMsg = ":hide was I supposed to be looking?";
                            postToRoom(RoomMsg);
                        }
                    }
                }
            }
        }
        function postNope(msgSpan) {
            if (postNo){
                var poster;
                //is the message from the model?
                var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
                if (nameSpan) {
                    poster = nameSpan.innerHTML.replace(/:/, '');
                    if (poster === currentModelName) {}
                    else {
                        RoomMsg = ":doano " + poster;
                        postToRoom(RoomMsg);
                    }
                }
            }
            else{}
        }
        function addBanned(bana, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            var banee = bana[1];
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster == "TestCounter")  || (poster == "Parkesy86")) {
                    if (banned.indexOf(banee) == -1) {
                        banned.push(banee);
                        RoomMsg = banee + " successfully add to countr banlist and will unable to be room helper";
                        postToRoom(RoomMsg);
                        ParkMsg = banee + " has been banned from countr";
                        postToPark(ParkMsg);
                    }
                    else {
                        RoomMsg = banee + " already appears to be on countrs ban list";
                        postToRoom(RoomMsg);
                        ParkMsg = banee + " is already banned from countr";
                    }
                }
                else if (poster === currentModelName){
                    if (banned.indexOf(banee) == -1) {
                        banned.push(banee);
                        RoomMsg = banee + " successfully add to countr banlist and the room";
                        postToRoom(RoomMsg);
                        RoomMsg = "/ban " + banee;
                        postToRoom(RoomMsg);
                        ParkMsg = banee + " has been banned from countr";
                        postToPark(ParkMsg);
                    }
                    else {
                        RoomMsg = banee + " already appears to be on countrs ban list";
                        postToRoom(RoomMsg);
                        ParkMsg = banee + " is already banned from countr";
                        postToPark(ParkMsg);
                    }
                }
                else {
                    RoomMsg = poster + "I'm sorry but you can't ban people, nice try though";
                    postToRoom(RoomMsg);
                }
            }
        }
        function removeBanned(banner, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            var banee = banner[1];
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster == "TestCounter")  || (poster == "Parkesy86")) {
                    var a = banned.indexOf(banee);
                    if (0 > a) {
                        RoomMsg = "I'm sorry but " + banee + " is not on my ban list, check PM for ban list";
                        postToRoom(RoomMsg);
                        ParkMsg = (banned.sort());
                        postToPark(ParkMsg);
                    }
                    else {
                        banned = [];
                        RoomMsg = "Success " + banee + " has been removed from the countr ban list and can be assigned as a helper";
                        postToRoom(RoomMsg);
                        ParkMsg = banned + " is the new list of banned people";
                        postToPark(ParkMsg);
                    }
                }
                else if (poster === currentModelName) {
                    RoomMsg = "I'm sorry " + poster + " you can't remove people from countr's ban list";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " has tried to remove " + banee + " from the ban list";
                    postToPark(ParkMsg);
                }
                if (banned.indexOf(poster) >= 0) {
                    RoomMsg = poster + " you were banned for trying to gain information you shouldn't and are now trying to do it again!";
                    postToRoom(RoomMsg);
                    ParkMsg = banned + " has tried to remove themselves from the ban list";
                    postToPark(ParkMsg);
                }
                else if (poster === helper) {
                    RoomMsg = "/pm " + helper + " this request is beyond your capabilities, the operator has been notified!";
                    postToRoom(RoomMsg);
                    RoomMsg = "helper tried to request information they shouldn't, as such has been removed as helper and banned for 24hrs";
                    postToRoom(RoomMsg);
                    isHelperAssigned = false;
                    if (banned.indexOf(poster) == -1) {
                        banned.push(poster);
                    }
                    ParkMsg = poster + " tried to unban " + banee;
                    postToPark(ParkMsg);
                }
            }
        }
        function addHighTip(addHighest, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                var amount = parseInt(addHighest[2]);
                var contributor = addHighest[1];
                poster = nameSpan.innerHTML;
                if ((poster == ("PM from " + currentModelName)) || (poster == "PM from Parkesy86") || (poster == "PM from TestCounter")) {
                    RoomMsg = "/pm " + poster + " highest tipper has been updated";
                    postToRoom(RoomMsg);
                    addContributor(contributor, amount);
                    ParkMsg = poster + " has added " + contributor + " to highest tip with " + amount;
                    postToPark(ParkMsg);
                    if (amount > highestTip) {
                        highestTip = amount;
                        setElementText(spanHighestTip, contributor + " (" + highestTip + ")");
                    }
                }
                else if (poster == ("PM from " + helper)) {
                    if (banned.indexOf(helper) == -1) {
                        banned.push(helper);}
                    RoomMsg = "helper removed from countr";
                    postToRoom(RoomMsg);
                    RoomMsg = "/pm " + helper + " this request is beyond your capabilities, the operator has been notified!";
                    postToRoom(RoomMsg);
                    ModelMsg  = helper + "tried to request information they shouldn't, as such has been removed as helper and banned for 24hrs";
                    postToModel(ModelMsg);
                    isHelperAssigned = false;
                    ParkMsg = helper + " tried to add highest tip information";
                    postToPark(ParkMsg);
                }
                else {
                    RoomMsg = poster + " I'm sorry only the model can update from ninja's";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to add to highest tip with " + amount + ", " + contributor;
                    postToPark(ParkMsg);
                }
            }
        }
        function postToPark(ParkMsg) {
            postChatMessage("/pm parkesy86 " + ParkMsg);
        }
        function postToModel(ModelMsg) {
            postChatMessage("/pm " + currentModelName + " " + ModelMsg);
        }
        function postToRoom(RoomMsg) {
            postCountTimeout = setTimeout(function () {
                postCountTimeout = null;
                postChatMessage(RoomMsg);
            }, 5000);
        }
        function removeHelper(helpRmv, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                var removeVal = helpRmv[1];
                if ((poster === currentModelName) || (poster == "TestCounter")  || (poster == "Parkesy86")) {
                    if (isHelperAssigned) {
                        if (removeVal === helper) {
                            isHelperAssigned = false;
                            helper = [];
                            RoomMsg = "helper has been removed and can no longer control :countr";
                            postToRoom(RoomMsg);
                        }
                        else {
                            RoomMsg = removeVal + " is not designated as helper, " + helper + " is designated as helper";
                            postToRoom(RoomMsg);
                        }
                    }
                    else {
                        RoomMsg = "helper is not assigned";
                        postToRoom(RoomMsg);
                    }
                }
                else if (poster === helper) {
                    RoomMsg = "I'm sorry " + helper + " you can not remove yourself as helper";
                    postToRoom(RoomMsg);
                }
                else {
                    RoomMsg = "I'm sorry " + poster + " you can not remove helpers";
                    postToRoom(RoomMsg);
                    ParkMsg = poster + " tried to remove " + removeVal + " from helper, helper is actually " + helper;
                    postToPark(ParkMsg);
                }
            }
        }
        function removeSpam(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((poster === currentModelName) || (poster == "Parkesy86")  || (poster == "TestCounter")  || (poster == "HoleeeSheeet") || (poster === helper) || (poster == "AnaliasPrince") || (poster == "AnaliasWes") || (poster == "AnaliasPony") || (poster == "AnaliasPhil") || (poster == "AnaliasLole")) {
                    postChatMessage("/pm " + poster + " removing spam as requested ");
                    postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                    postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                    postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                    postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                    postChatMessage(":nocrude");
                }
            }
        }
        function handleKill(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = msgSpan.parentNode.querySelector('a span.name_model') || msgSpan.parentNode.querySelector('a span.name_premium');
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if (currentModelName === poster) {
                    killPosting = true;
                    localStorage['mfca_killposting_' + currentModelName] = 'true';
                }
            }
        }
        function startCountDown() {
            if (isCountDownActive) {
                if (!confirm("You already have a countdown running.  Override?")) {
                    return;
                }
            }
            var newCountDown = prompt("countdown calculated using 'Topic1-5' in the script", 1000);
            if (null === newCountDown) {
                return; //canceled out
            }
            if (NaN !== parseInt(newCountDown)) {
                countDown = newCountDown;
                isCountDownActive = true;
                showCurrentCountDown();
                postCurrentCount();
                aAutoPost.setAttribute('class', 'show');
                aPostCurrentCount.setAttribute('class', 'show');
            }
        }
        var postCountQueued = false;
        function RemoveRubbish(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage("."); postChatMessage(".");
                postChatMessage(":mno " + poster + " good try, take your viruses elsewhere");
            }
        }

        //Handling Tips & Countdowns
        function handleTip(tip, tipSpan) {
            if (tipSpan.getAttribute('class').indexOf('chat_system') === -1) return;
            //parse the values
            var amount = parseInt(tip[2]);
            var contributor = tip[1];
            addContributor(contributor, amount);
            setElementText(spanTopContrib, contributors[0].contributor + " (" + contributors[0].amount + ")");
            //modify the tip span so we can visually see it was processed
            tipSpan.setAttribute('class', 'processed');
            //calculate countdown values if needed
            if (PostTips) {
                ModelMsg = contributor + " has tipped " + amount + " tokens";
                postToModel(ModelMsg);
            }
            if (isGuessActive) {
                PlayGuessGame(amount, contributor);
                if (isCountDownActive) {
                    adjustCountDown(amount);
                }
            }
            else if (amount === WarTip) {
                if (isWarActive) {
                    RoomMsg = "shuffling deck, please wait....... :deckshuffle ";
                    postToRoom(RoomMsg);
                    postCountTimeout = setTimeout(function () {
                        postCountTimeout = null;
                        StartTipWar(contributor);
                    }, 5000);
                }
                if (isCountDownActive) {
                    adjustCountDown(amount);
                }
            }
            else if (amount === BlackJackTip) {
                if (isBlackJackActive) {
                    if (isGameInProgress) {
                        RoomMsg = "BlackJack game is in progress, room operator will run a manual game once this one has ended";
                        postToRoom(RoomMsg);
                        ParkMsg = "Please start manual BlackJackgame for " + contributor + " once this one has ended";
                        postToPark(ParkMsg);
                    }
                    else {
                        isGameInProgress = true;
                        var play = contributor;
                        BlackJackInitialHand(play);
                    }
                    if (isCountDownActive) {
                        adjustCountDown(amount);
                    }
                }
            }
            else if (amount === SpinTip) {
                if (isSpinActive) {
                    SpinTheWheel(contributor);
                }
                if (isCountDownActive) {
                    adjustCountDown(amount);
                }
            }
            else if (amount === SpankSpec) {
                adjustTopicSpecials(amount, contributor);
            }
            else if (amount === FlashSpec) {
                adjustTopicSpecials(amount, contributor);
            }
            else if (amount === SnapSpec) {
                adjustTopicSpecials(amount, contributor);
            }
            else if (amount === VidSgl) {
                adjustTopicSpecials(amount, contributor);
            }
            else if (amount === VidDbl) {
                adjustTopicSpecials(amount, contributor);
                if (CountrSpec) {
                    if (CountrSpec1) {
                        if (CountrSpec2) {
                            if (CountrSpec3) {
                                CountrSpec3 = false;
                                RoomMsg = "/tip 250  CountrSpecial for " + contributor;
                                postToRoom(RoomMsg);
                                RoomMsg = "Just 2 more 250tk contributions remain!!";
                                postToRoom(RoomMsg);
                            }
                            else {
                                CountrSpec2 = false;
                                RoomMsg = "/tip 250  CountrSpecial for " + contributor;
                                postToRoom(RoomMsg);
                                RoomMsg = "Just 1 more 250tk contribution remains!!";
                                postToRoom(RoomMsg);
                            }
                        }
                        else {
                            CountrSpec1 = false;
                            CountrSpec = false;
                            RoomMsg = "/tip 250  CountrSpecial for " + contributor;
                            postToRoom(RoomMsg);
                            RoomMsg = ":wowo No more 250tk contributions remain :sadkid";
                            postToRoom(RoomMsg);
                        }
                    }
                }
            }
            else if (amount === Vid1) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount === Vid2) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount === Vid3) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount === Vid4) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount === Vid5) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount === Vid6) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount === Vid7) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount === Vid8) {
                adjustVideoSpecials(amount, contributor);
            }
            else if (amount >= 501) {
                QuestionAmount = amount;
                requestForTopic(QuestionAmount, contributor);
            }
            else {
                if (isCountDownActive) {
                    adjustCountDown(amount);
                }
            }
            //add stats
            if (amount > highestTip) {
                highestTip = amount;
                setElementText(spanHighestTip, contributor + " (" + highestTip + ")");
            }
        }
        function adjustTopicSpecials(amount, contributor) {
            if (amount === SpankSpec) {
                ModelMsg = " a da o palma pentru " + contributor;
                postToModel(ModelMsg);
                RoomMsg = ":doaspank1 " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === VidDbl) {
                ModelMsg = " vid sale for " + contributor;
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === VidSgl) {
                ModelMsg = " g/g vid for " + contributor;
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === VidTrp) {
                ModelMsg = " vid sale >2 for " + contributor;
                postToModel(ModelMsg);
            }
            else if (amount === FlashSpec) {
                ModelMsg = " expune pentru " + contributor;
                postToModel(ModelMsg);
                RoomMsg = ":lppa " + contributor;
                postToRoom(RoomMsg);
                if (isCountDownActive) {
                    adjustCountDown(amount);
                }
            }
            else if (amount === SnapSpec) {
                ModelMsg = "SnapChat prentru " + contributor;
                postToModel(ModelMsg);
            }
        }
        function adjustVideoSpecials(amount, contributor) {
            if (amount === Vid1) {
                ModelMsg = contributor + " a trimis pentru film #1 or #2 or #3";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === Vid2) {
                ModelMsg = contributor + " a trimis pentru film #4";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === Vid3) {
                ModelMsg = contributor + " a trimis pentru film #7";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === Vid4) {
                ModelMsg = contributor + " a trimis pentru film #8";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === Vid5) {
                ModelMsg = contributor + " a trimis pentru films #5 or #9 or #10";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === Vid6) {
                ModelMsg = contributor + " a trimis pentru film #11";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === Vid7) {
                ModelMsg = contributor + " a trimis pentru film #12";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
            else if (amount === Vid8) {
                ModelMsg = contributor + " a trimis pentru film #6";
                postToModel(ModelMsg);
                RoomMsg = ":cclap " + contributor;
                postToRoom(RoomMsg);
            }
        }
        function adjustCountDown(amount) {
            if (isCountDownActive) {
                updateCountDown(amount);
            }
        }
        function updateCountDown(amount) {
            if (IsTopic1Active) {
                countDown = countDown - amount;
                if (countDown <= 0) {
                    maAlert("Topic 1 Complete!");
                    countDown = 0;
                    isCountDownActive = false;
                    IsTopic1Active = false;
                    ModelMsg = "urmatoral topic: " + Topic2;
                    postToModel(ModelMsg);
                    ModelMsg = " reply with 'start topic 2' to begin next topic";
                    postToModel(ModelMsg);
                    autoPostActive = true;
                }
                else {
                    showCurrentCountDown();
                    //post the count if auto post is active
                    if (autoPostActive)
                        postCurrentCount();
                }
            }
            else if (IsTopic2Active) {
                countDown = countDown - amount;
                if (countDown <= 0) {
                    maAlert("Topic 2 Complete!");
                    countDown = 0;
                    IsTopic2Active = false;
                    isCountDownActive = false;
                    ModelMsg = "urmatoral topic: " + Topic3;
                    postToModel(ModelMsg);
                    ModelMsg = " reply with 'start topic 3' to begin next topic";
                    postToModel(ModelMsg);
                    autoPostActive = true;
                }
                else {
                    showCurrentCountDown();
                    //post the count if auto post is active
                    if (autoPostActive)
                        postCurrentCount();
                }
            }
            else if (IsTopic3Active) {
                countDown = countDown - amount;
                if (countDown <= 0) {
                    maAlert("Topic 3 Complete!");
                    countDown = 0;
                    isCountDownActive = false;
                    IsTopic3Active = false;
                    ModelMsg = "urmatoral topic: " + Topic4;
                    postToModel(ModelMsg);
                    ModelMsg = " reply with 'start topic 4' to begin next topic";
                    postToModel(ModelMsg);
                    autoPostActive = true;
                }
                else {
                    showCurrentCountDown();
                    //post the count if auto post is active
                    if (autoPostActive)
                        postCurrentCount();
                }
            }
            else if (IsTopic4Active) {
                countDown = countDown - amount;
                if (countDown <= 0) {
                    maAlert("Topic 4 Complete!");
                    countDown = 0;
                    isCountDownActive = false;
                    IsTopic4Active = false;
                    ModelMsg = "urmatoral topic: " + Topic5;
                    postToModel(ModelMsg);
                    ModelMsg = " reply with 'start topic 5' to begin next topic";
                    postToModel(ModelMsg);
                    autoPostActive = true;
                }
                else {
                    showCurrentCountDown();
                    //post the count if auto post is active
                    if (autoPostActive)
                        postCurrentCount();
                }
            }
            else if (IsTopic5Active) {
                countDown = countDown - amount;
                if (countDown <= 0) {
                    maAlert("Topic 5 Complete!");
                    countDown = 0;
                    isCountDownActive = false;
                    IsTopic5Active = false;
                    ModelMsg = "urmatoral topic: " + FinalTopic;
                    postToModel(ModelMsg);
                    ModelMsg = " reply with 'no more topic' to post next topic";
                    postToModel(ModelMsg);
                    autoPostActive = true;
                }
                else {
                    showCurrentCountDown();
                    //post the count if auto post is active
                    if (autoPostActive)
                        postCurrentCount();
                }
            }
            else {
                ModelMsg = "No count is currently running";
                postToModel(ModelMsg);
            }
        }
        function addContributor(contributor, amount) {
            var bFound = false;
            for (var i = 0; i < contributors.length; i++) {
                if (contributors[i].contributor === contributor) {
                    contributors[i].amount += amount;
                    bFound = true;
                }
            }
            if (!bFound) {
                contributors.push({ contributor: contributor, amount: amount });
            }
            contributors.sort(function (a, b) { return b.amount - a.amount; });
        }
        function modelChanged(modelId) {
            setupHTML();
            //make sure we entered a models room (and not a lounge or the homepage)
            if (undefined !== unsafeWindow.t.g_hLoungeIds[modelId] || null == modelId) {
                document.querySelector('#ma-menu').setAttribute('class', 'hidden');
                return;
            }
            currentModelName = unsafeWindow.t.g_hUsers[modelId]["username"];
            document.querySelector('#ma-menu').setAttribute('class', 'show');
            //tip & kill switch
            tipRegex = "(.[^:]+) has tipped $$model$$ (.*) tokens.".replace("$$model$$", currentModelName);
            killRegex = "^the soap is a lye$";

            //simple topic controls
            ChangePostingRegex = /post countr/i;
            AdjustCountRegex = /(.*)/i;
            TopicUpdateRegex = "^Topic: Hi Guys! (.*)tk(.[^:]+)$";
            postTipRegex = /change posting/i;
            postRegex = /topic! please/i;
            startRegex = /start topic ([1-5])/i;
            helpRegex = /countr! help/i;
            stopRegex = /stop/i;
            countrRegex = /which version!/i;
            getlastRegex = /my topic/i;
            ShowRegex = /change (.[^:]+)/i;
            topicValRegex = /T1 (.*)/i;
            SpecialRegex = /adjust (spank|flash|vidsgl|viddbl|snap|vidtrp|vidamt|varvid) (.*)/i;
            highTipRegex = /highest! tip?/i;
            StopTopicRegex = /next! topic/i;
            TipHerRegex = /!tip!/i;
            PostTopSpec = /!special!/;
            PvtRegex = /Her last private show (.[^:]+)/i;
            noTopicRegex = /no more topics/i;
            CountrRegex = /Hi Countr/i;

            //GameRegex
            gameRegex = /game! is/i;
            stopGamesRegex = /stop all!/i;
            startGameRegex = /(start|stop) (war|guess|blackjack|spin|special)!/i;
            setGameTipRegex = /set (war|blackjack|spin)(.*) tip!/i;
            setGamePrizeRegex = /(war|guess|blackjack|spin) prize is (.[^:]+) x/i;

            //Guessing Game
            toggleGuessRegex = /clear guess! game/i;
            addgameRegex = /add (.*) to numbers!/i;

            //BlackJack Game
            BlkJk1Regex = /blackjack for (.[^:]+)/i;
            BlkJk2Regex = /(draw!|stick!|stay!)/i;

            //Spin The Wheel Game
            SpinRegex = /Spin for (.[^:]+)/i;

            //War Game
            playWarRegex = /play war! (.[^:]+)/i;

            //Advanced Countr commands
            TestModeRegex = /test mode (on|off)/i;
            spoofTipRegex = /test tip (.*) tokens/i;
            assignHelperRegex = /rha (.[^:]+)/i;
            removeHelperRegex = /rhb (.[^:]+)/i;
            checkHelperRegex = /helper!/i;
            addHighRegex = /(.[^:]+) with (.*) highest tip/i;
            bannedRemoveRegex = /remove! (.[^:]+)/i;
            addBannedRegex = /add! (.[^:]+)/i;
            toggleNoRegex = /no command (on|off)/i;
            spamRegex = /!spam!/i;
            SaidHi = [];

            //Lookout through chat
            RequestRegex = /(yes|no)/i;
            nopeRegex = /(babe[ !?"':,]|baby[ !?"':,]| bb[ !?"':,]|anal[ !?"':,]|skype[ !?"':,]|group[ !?"':,]|mmmmm[ !?"':,])/i;

            //set prizes
            BlackJackPrize = [];
            WarPrize = [];
            GuessPrize = [];
            SpinPrize = [];

            //set game values
            SpinTip = 0;
            BlackJackTip = 0;
            WarTip = 0;
            chance = 0;
            isGameInProgress = false;

            //set assigned values
            SpinPrizeAssigned = false;
            BlackJackPrizeAssigned = false;
            WarPrizeAssigned = false;
            GuessPrizeAssigned = false;

            //set game activation status
            isSpinActive = false;
            isWarActive = false;
            isGuessActive = false;
            isBlackJackActive = false;
            CountrSpec = false;

            //Set War Info
            ModelCardLocation = 0;
            PlayerCardLocation = 0;
            WarPlayer = "";

            //set blackjack dealer and hand info
            NewModelHand = 0;
            NewPlayerHand = 0;
            ModelSet = false;
            PlayerSet = false;
            PlayerHand = 0;
            ModelHand = 0;
            AskedModelOnce = false;

            //assign guess game
            chanceGuessed = [];

            //Set Editable variables
            VidDbl = 555;
            SpankSpec = 99;
            VidSgl = 500;
            SnapSpec = 1717;
            FlashSpec = 100;
            Show1 = "Bra Off ";
            Show2 = "Panties Off ";
            Show3 = "Oil Show ";
            Show4 = "Ice Show ";
            Show5 = "Cream Show ";
            VidTrp = 5000;
            VidAmt = 2;
            VarVid = "G/G Vid ";
            Vid1 = 0;
            Vid2 = 0;
            Vid3 = 0;
            Vid4 = 0;
            Vid5 = 0;
            Vid6 = 0;
            Vid7 = 0;
            Vid8 = 0;
            topicVar2 = false;
            CountrSpec1 = false;
            CountrSpec2 = false;
            CountrSpec3 = false;
            postNo = false;
            banned = [];
            helpers = [];
            TipReq = false;
            countDown = 0;
            isCountDownActive = false;
            highestTip = 0;
            parsing = false;
            parseQueue = [];
            setElementText(spanHighestTip, 0);
            setElementText(spanTopContrib, 'none');
            setElementText(spanCurrentModel, currentModelName);
            setElementText(spanMenuCount, "");
            contributors = [];
            txtInput = document.querySelector('#message_input');
            btnSend = document.querySelector('#send_button');
            clearTimeout(postCountTimeout);
            autoPostActive = false;
            setElementText(aAutoPost, 'Enable AutoPost');
            aAutoPost.setAttribute('class', 'hidden');
            aPostCurrentCount.setAttribute('class', 'hidden');
            killPosting = localStorage['mfca_killposting_' + currentModelName] === 'true' ? true : false;
            //setup the parser
            document.querySelector('#chat_box').addEventListener('DOMNodeInserted', parseChatMessage);
        }
        function setElementText(e, text) {
            e.innerHTML = text;
        }
        function showElement(e, style) {
            e.style.display = style ? style : 'inline';
        }
        function hideElement(e) {
            e.style.display = 'none';
        }
        function toggleSound() {
            var soundEnabled = localStorage['mfca_soundEnabled'] ? localStorage['mfca_soundEnabled'] : 'disabled';
            if ('disabled' === soundEnabled) {
                document.querySelector('#soundToggleText').innerHTML = 'Disable Sounds';
                soundDiv.setAttribute('class', 'show');
                localStorage['mfca_soundEnabled'] = 'enabled';
            } else {
                document.querySelector('#soundToggleText').innerHTML = 'Enable Sounds';
                soundDiv.setAttribute('class', 'hidden');
                localStorage['mfca_soundEnabled'] = 'disabled';
            }
        }
        function configFriendSound(e) {
            var element = e.srcElement ? e.srcElement : e.target;
            localStorage['mfca_friendSound'] = element.value;
        }
        return {
            init: init
        }
    })();
    MAssist.init();
}
//set up the audio file
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/top.html')) {
    //these are the sounds in base64
    var mfcTip = '';
    var mfcFriendLogin = '';
    var mfcModelLogin = '';
    var sounds = [mfcModelLogin, mfcFriendLogin, mfcTip];
    //set up an HTML5 audio tag for playing sounds
    var audioTag = document.createElement('audio');
    document.body.appendChild(audioTag);
    //create the alert div for visual announcements
    var onlineDiv = document.createElement('div');
    onlineDiv.setAttribute('id', 'online-alert');
    onlineDiv.setAttribute('class', 'hidden');
    document.body.appendChild(onlineDiv);
    var tstStyles = [
        '    #online-alert ',
        '    {',
        '    	    position: absolute; ',
        '    	    top: 20px; ',
        '    	    width: 35%; ',
        '    	    height: 30px; ',
        '    	    left: 0; ',
        '    	    right: 0; ',
        '    	    margin-left: auto; ',
        '    	    margin-right: auto; ',
        '    	    background: White; ',
        '    	    text-align: center; ',
        '    	    font-family: Trebuchet MS; ',
        '    	    -moz-border-radius: 10px; ',
        '    	    -webkit-border-radius: 10px;',
        '    	    line-height:1.75em; ',
        '    } ',
        '    .hidden {display: none;} ',
        '    .show {display:block}'
    ];
    var GM_addStyle = function (s) {

    };
    GM_addStyle(tstStyles.join('\n'));
    //display/hide alert messages
    var onlineAlertTimeout;
    function onlineAlert(text) {
        clearTimeout(onlineAlertTimeout);
        onlineDiv.innerHTML = text;
        onlineDiv.setAttribute('class', 'show');
        onlineAlertTimeout = setTimeout(function () { onlineDiv.setAttribute('class', 'hidden'); }, 5000);
    }
    //setup friend management
    var onlineFriends = [], initialPop = false;
    setInterval(function () {
        //get the current friends list
        //        var currentFriends = Array.prototype.map.call(document.querySelectorAll('#friends #online_friends_list div[id^="online_friend_"]>a>span'), function (element) {
        //            return element.innerHTML;
        //        });
        var currentFriends = [];
        var g_hUsers = unsafeWindow.g_hUsers;
        for (key in g_hUsers) {
            if (g_hUsers[key] && g_hUsers[key]['friend'] == true) //filter for friends
                if (g_hUsers[key]['state'] != 127 && g_hUsers[key]['access_level'] == 4) //filter unknown video state
                    currentFriends.push(key);
        }
        //wait until we have an actual friends list
        if (!currentFriends || currentFriends.length === 0)
            return;
        //leave if this is the first time we got friends
        if (!initialPop) {
            initialPop = true;
            onlineFriends = currentFriends;
            return;
        }
        //see if there are any changes
        if (!compareArrays(currentFriends, onlineFriends)) {
            //compile new people
            var offlineMsg;
            var offline = diff(onlineFriends, currentFriends).map(function (item) { return g_hUsers[item]['username']; });
            if (offline.length > 0)
                offlineMsg = offline.join(", ") + " went offline.";
            //compile people that left
            var onlineMsg;
            var online = diff(currentFriends, onlineFriends).map(function (item) { return g_hUsers[item]['username']; });
            if (online.length > 0)
                onlineMsg = online.join(", ") + " came online.";
            //perform visual alert
            if (onlineMsg && offlineMsg)
                onlineAlert(onlineMsg + "  " + offlineMsg);
            else
                onlineAlert(onlineMsg ? onlineMsg : offlineMsg);
            //perform audio alert
            playOnlineSound();
            //save the current online list
            onlineFriends = currentFriends;
        }
    }, 1000);
    function playOnlineSound(type) {
        var soundEnabled = localStorage['mfca_soundEnabled'] ? localStorage['mfca_soundEnabled'] : 'disabled';
        if ('enabled' === soundEnabled) {
            var friendSound = parseInt(['mfca_friendSound'] ? localStorage['mfca_friendSound'] : 0, 10);
            audioTag.setAttribute('src', 'data:audio/ogg;base64,' + sounds[friendSound]);
            audioTag.play();
        }
    }
}
function createTable(spec) {
    var table = document.createElement('table');
    for (var row = 0; row < spec.length; row++) {
        var tr = document.createElement('tr');
        for (var cell = 0; cell < spec[row].length; cell++) {
            var td = document.createElement('td');
            if ("string" === typeof (spec[row][cell])) {
                td.appendChild(document.createTextNode(spec[row][cell]));
            } else {
                td.appendChild(spec[row][cell]);
            }
            tr.appendChild(td);
        }
        table.appendChild(tr);
    }
    return table;
}
function compareArrays(orig, compare) {
    //if the lengths are not the same, the arrays are not the same
    if (orig.length != compare.length) return false;

    for (var i = 0; i < compare.length; i++) {
//        if (isArray(orig[i])) { //likely nested array
//            if (!compareArrays(orig[i], compare[i])) return false;
//            else continue;
//        }
        if (orig[i] != compare[i]) return false;
    }
    return true;
}
function diff(orig, compare) {
    return orig.filter(function (i) { return !(compare.indexOf(i) > -1); });
}
function isArray(obj){
    return toString.call(obj) === "[object Array]";
}
