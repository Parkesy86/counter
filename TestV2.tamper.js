...
if (0 === window.location.href.indexOf('http://www.myfreecams.com/mfc2/static/player.html')) {
    ...
    var MAssist = (function () {
        var htmlSetup = false, currentModelID = "", findPlayer, stopRegex, minusRegex, plusRegex, postRegex, currentModelName;
        var countDown, contributors = [], isCountDownActive, tipRegex, alertTimeout, killRegex, highestTip = 0;
        var Video1, Video2, Video3, Video4, Video5, Video6, Video7, Video8, VideoSpecial, NDeduct, noRegex;
        var AllowVideo1, AllowVideo2, AllowVideo3, AllowVideo4, AllowVideo5, AllowVideo6, AllowVideo7, AllowVideo8, AllowVideoSpec;
        var AllowSpecial1, AllowSpecial2, AllowSpecial3, AllowSpecial4, AllowSpecial5, AllowSpecial6, wilRegex, parkRegex;
        var parsing = false, parseQueue = [], autoPostActive = false, postCountTimeout = null, TwitMsg, VidMsg, HoldingTopic;
        ...
        var Topic1, Topic2, Topic3, Topic4, Topic5, T1, T2, T3, T4, T5, TpcSpec1, TpcSpec2, TpcSpec3, TpcSpec4, TpcSpec5, PMValue, TpcSpec6;
        var mfcLoadPlayer, TopicText1, TopicText2, TopicText3, TopicText4, TopicText5, TpcSpecTxt1, TpcSpecTxt2, TpcSpecTxt3, TpcSpecTxt4, TpcSpecTxt5, TpcSpecTxt6;
        var topiconeRegex, topictwoRegex, topicthreeRegex, topicfourRegex, topicfiveRegex, topicRegex, VidTip, helpRegex;
        function init() {
            setTimeout(function () {
        ...
        function setupHTML() {
            ...
            var txtMenuText = document.createTextNode('AnaliasCountr TestV2');
...
//set the topic amounts
        Topic1 = 2555;
        Topic2 = 2555;
        Topic3 = 1999;
        Topic4 = 1999;
        Topic5 = 1999;
//set the auto adjustment values
        T1 = (Topic1 + Topic2 + Topic3 + Topic4 + Topic5);
        T2 = (Topic2 + Topic3 + Topic4 + Topic5);
        T3 = (Topic3 + Topic4 + Topic5);
        T4 = (Topic4 + Topic5);
        T5 = Topic5;
//set the topic specials
//allow topic specials to add to topic? (change allowspecial# to true to count to topic or false to not count)
//not deducted message
...
//allow video tips to add to topic? (change allowvideo# to true to count to topic or false to not count)
        ...
        function showCurrentCountDown() {
            //maAlert("AnaliasCountr: " + countDown + " tokens left on countdown.");
        ...
        function postCurrentTopic() {
            if (countDown >= T1) {
                ...
                postChatMessage(PMValue + TopicText1);
            }
            else if (countDown >= (1 + T3)) {
                postChatMessage("/topic " + TopicText2);
                postChatMessage(PMValue + TopicText2);
            }
            else if (countDown >= (1 + T4)) {
                postChatMessage("/topic " + TopicText3);
                postChatMessage(PMValue + TopicText3);
            }
            else if (countDown >= (1 + T5)) {
                postChatMessage("/topic " + TopicText4);
                postChatMessage(PMValue + TopicText4);
            }
            else if (countDown >= 1) {
                postChatMessage("/topic " + TopicText5);
                postChatMessage(PMValue + TopicText5);
            }
            else {
                postChatMessage("/topic " + HoldingTopic);
                postChatMessage(PMValue + HoldingTopic);
            }
        }
        function postCurrentCount() {
            ...
            var CDAmt1 = countDown - T2;
            var CDAmt2 = countDown - T3;
            var CDAmt3 = countDown - T4;
            var CDAmt4 = countDown - T5;
            var CDAmt5 = countDown;
            //edit the output messages here
            //ensure charachter count for topics are 144 characters or less (after the $$value$$ has been replaced with count)
            TopicText1 = ("Hi Guys! $$value$$tk Bra Off, 99 Spank/100 Flash, 1717 SnapChat 4ever, Vid sale 4 vids-999tk(u choose from profile) G/G vid-500tk Ty guys").replace("$$value$$", CDAmt1);
            TopicText2 = ("Hi Guys! $$value$$tk panties Off, 99spank/100 Flash, 1717 SnapChat 4ever, Vid sale 4 vids-999tk(u choose from profile) G/G vid-500tk Ty guys").replace("$$value$$", CDAmt2);
            TopicText3 = ("Hi Guys, $$value$$ Oil Show, 99tk Spank, 1717 SnapChat 4ever, 500tk new g/g vid, 400tk solo vacation vid, more vids on profile").replace("$$value$$", CDAmt3);
            TopicText4 = ("Hi Guys, $$value$$ Cream Show, 99tk Spank, 1717 SnapChat 4ever, 500tk new g/g vid, 400tk solo vacation vid, more vids on profile").replace("$$value$$", CDAmt4);
            TopicText5 = ("Hi Guys, $$value$$ Ice Show, 99tk Spank, 1717 SnapChat 4ever, 500tk new g/g vid, 400tk solo vacation vid, more vids on profile").replace("$$value$$", CDAmt5);
            HoldingTopic = ("Hi Guys, Tip if you like, 99tk Spank, 1717tk SnapChat 4ever, 500tk new g/g vid, 400tk solo vacation vid, more vids on profile");
            PMValue = "/pm AnaliasCountr ".replace("$$model$$", currentModelName);
            TwitMsg = "make sure to follow $$model$$ on Twitter :doatwit".replace("$$model$$", currentModelName);
            VidMsg = "all videos listed on http://profile.myfreecams.com/DreamOfAnalia";
            if (countDown >= T1) {
                ...
                postChatMessage("/topic " + TopicText1);
                postChatMessage(PMValue + " if counter is wrong, just remove it from being room helper and it will not update topic");
                postChatMessage(VidMsg);
                postChatMessage(TwitMsg);
            }
            else if (countDown >= (1 + T2)) {
                postChatMessage(PMValue + (countDown - T2));
                postChatMessage("/topic " + TopicText1);
            }
            else if (countDown >= (1 + T3)) {
                postChatMessage(PMValue + (countDown - T3));
                postChatMessage("/topic " + TopicText2);
            }
            else if (countDown >= (1 + T4)) {
                postChatMessage(PMValue + (countDown - T4));
                postChatMessage("/topic " + TopicText3);
            }
            else if (countDown >= 1) {
                postChatMessage(PMValue + (countDown - T5));
                postChatMessage("/topic " + TopicText4);
            }
            else {
                postChatMessage(PMValue + countDown);
            }
        ...
        function parseChatMessage() {
            ...
            while (parseQueue.length > 0) {
                ...
                if ("true" === msg.getAttribute('ma-processed'))
                    continue;
                if (msg.contains("skype") || msg.contains("bb") || msg.contains("anal")) {
                    postChatMessage(":doano");
                }
                var nobb = msg.innerHTML.match(helpRegex);
                if(nobb)
                    postChatMessage(":doano");
                //chat commands?
                var helprequest = msg.innerHTML.match(helpRegex);
                if (helprequest)
                    help(helprequest, msg);
                var start1 = msg.innerHTML.match(topiconeRegex);
                if (start1)
                    count1(start1, msg);
                var start2 = msg.innerHTML.match(topictwoRegex);
                if (start2)
                    count2(start2, msg);
                var start3 = msg.innerHTML.match(topicthreeRegex);
                if (start3)
                    count3(start3, msg);
                var start4 = msg.innerHTML.match(topicfourRegex);
                if (start4)
                    count4(start4, msg);
                var start5 = msg.innerHTML.match(topicfiveRegex);
                if (start5)
                    count5(start5, msg);
                var stopcmd = msg.innerHTML.match(stopRegex);
                if (stopcmd)
                    stopCount(stopcmd, msg);
                var postcmd = msg.innerHTML.match(postRegex);
                if (postcmd)
                    postCurrentCount(postcmd, msg);
                var pluscmd = msg.innerHTML.match(plusRegex);
                if (pluscmd)
                    handlePlus(pluscmd, msg);
                var minuscmd = msg.innerHTML.match(minusRegex);
                if (minuscmd)
                    handleMinus(minuscmd, msg);
                var topiccmd = msg.innerHTML.match(topicRegex);
                if (topiccmd)
                    postCurrentTopic();
                //is this a tip?
        ...
        function help(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium name_other')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((currentModelName === poster) || ("Parkesy86" === poster) || ("AnaliasCountr" === poster) || ("TestCounter" === poster) || ("HoleeeSheeet" === poster)) {
                    postChatMessage("!topic1, !topi2, !topic3, !topic4, !topic5, !help, !stop, !post, !plus xxx, !minus xxx, topic?");
                }
                else {
                    postChatMessage("I'm sorry, you do not have authority to manipulate countr :nerner")
                }
            }
        }
        function count1(msgSpan) {
            var poster;
            //is the message from the model?
            ...
            if (nameSpan) {
                ...
                if ((currentModelName === poster) || ("Parkesy86" === poster) || ("AnaliasCountr" === poster) || ("TestCounter" === poster) || ("HoleeeSheeet" === poster)) {
                    ...
                    autoPostActive = true;
                    isCountDownActive = true;
                    postChatMessage("countdown started as requested :tiphat");
                    postCurrentCount();
                    postCurrentTopic();
                }
            }
        }
        function count2(msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
            ...
            if (nameSpan) {
                ...
                if ((currentModelName === poster) || ("Parkesy86" === poster) || ("AnaliasCountr" === poster) || ("TestCounter" === poster) || ("HoleeeSheeet" === poster)) {
                    if (isCountDownActive) {
                        ...
                        countDown = 0;
                        autoPostActive = false;
                        postChatMessage("topic cleared  as requested")
                    }
                    else {
                        postChatMessage("no countdown running, please start one")
                    }
                }
            }
        }
        function handlePlus(pluscmd, pluscmdSpan, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((currentModelName === poster) || ("Parkesy86" === poster) || ("AnaliasCountr" === poster) || ("TestCounter" === poster) || ("HoleeeSheeet" === poster)) {
                    if (pluscmdSpan.getAttribute('class').indexOf('chat_premium') === -1) {
                        //parse the values
                        var plusamt = parseInt(pluscmd[1]);
                        if (isCountDownActive) {
                            countDown = countDown + plusamt;
                            postCurrentCount();
                            postCurrentTopic();
                            postChatMessage("topic increased by " + plusamt + " tokens");
                        }
                        else {
                            postChatMessage("no countdown running, unable to adjust")
                        }
                    }
                }
            }
        }
        function handleMinus(minuscmd, minuscmdSpan, msgSpan) {
            var poster;
            //is the message from the model?
            var nameSpan = ((msgSpan.parentNode.querySelector('a span.name_model')) || (msgSpan.parentNode.querySelector('a span.name_premium')));
            if (nameSpan) {
                poster = nameSpan.innerHTML.replace(/:/, '');
                if ((currentModelName === poster) || ("Parkesy86" === poster) || ("AnaliasCountr" === poster) || ("TestCounter" === poster) || ("HoleeeSheeet" === poster)) {
                    if (minuscmdSpan.getAttribute('class').indexOf('chat_premium') === -1) {
                        //parse the values
                        var minusamt = parseInt(minuscmd[1]);
                        if (isCountDownActive) {
                            if ((countDown - minusamt) >= 0) {
                                postChatMessage("this will reduce topic to 0 or belowm please use stop command & then set the topic using the topic command to begin a new one");
                            }
                            else {
                                countDown = countDown - minusamt;
                                postCurrentCount();
                                postCurrentTopic();
                                postChatMessage("topic reduced by " + minusamt + " tokens");
                            }
                        }
                        else {
                            postChatMessage("no countdown running, unable to adjust")
        ...
        function handleTip(tip, tipSpan) {
            ...
            VidTip = PMValue + contributor + " a trimis " + amount + " pentru un film";
            //calculate countdown values if needed
            if (isCountDownActive) {
                if (contributor === "Parkesy86") {
                    postChatMessage("/pm Parkesy86 sorry I can't count your tips to topic")
                }
                else if (contributor === "wilnc") {
                    adjustCountDown(amount)
                }
                else if (amount === TpcSpec1) {
                    adjustTopicSpecials(amount, contributor);
                }
                else if (amount === TpcSpec2) {
                    adjustTopicSpecials(amount, contributor);
                }
                else if (amount === TpcSpec3) {
                    adjustTopicSpecials(amount, contributor);
                }
                else if (amount === TpcSpec4) {
                    adjustTopicSpecials(amount, contributor);
                }
                else if (amount === TpcSpec5) {
                    adjustTopicSpecials(amount, contributor);
                }
                else if (amount === TpcSpec6) {
                    adjustTopicSpecials(amount, contributor);
                }
                else if (amount === Video1) {
                    if (AllowVideo1) {
                        postChatMessage(VidTip);
                        adjustCountDown(amount);
                    }
                    else {
                        postChatMessage(PMValue + NDeduct);
                        postChatMessage(VidTip);
                    }
                }
                else if (amount === Video2) {
                    if (AllowVideo2) {
                        postChatMessage(VidTip);
                        adjustCountDown(amount);
                    }
        ...
        function adjustTopicSpecials(amount, contributor) {
            if (amount === TpcSpec1) {
                if (AllowSpecial1) {
                    postChatMessage(PMValue + TpcSpecTxt1 + contributor);
                ...
                if (AllowSpecial2) {
                    postChatMessage(PMValue + TpcSpecTxt2 + contributor);
                ...
                if (AllowSpecial3) {
                    postChatMessage(PMValue + TpcSpecTxt3 + contributor);
                    postChatMessage(":lppa " + contributor);
                    adjustCountDown(amount)
                }
                else {
                ...
                if (AllowSpecial4) {
                    postChatMessage(PMValue + TpcSpecTxt4 + contributor);
                    postChatMessage(":lppa " + contributor);
                    adjustCountDown(amount)
                }
                else {
                ...
                if (AllowSpecial5) {
                    postChatMessage(PMValue + TpcSpecTxt5 + contributor);
                    postChatMessage(":lppa " + contributor);
                    adjustCountDown(amount)
                }
                else {
                ...
                if (AllowSpecial6) {
                    postChatMessage(PMValue + TpcSpecTxt6 + contributor);
                    postChatMessage(":lppa " + contributor);
                    adjustCountDown(amount)
                }
                else {
        ...
        function updateCountDown1(amount) {
            ...
            if ((countDown - T2) <= 0) {
                ...
                autoPostActive = true;
                postChatMessage("/topic " + TopicText2);
                postChatMessage(":minionboobies");
                postChatMessage(TwitMsg);
                postChatMessage(PMValue + " urmatoral topic: " + TopicText2);
                postChatMessage(VidMsg);
                showCurrentCountDown();
            }
            else {
        ...
        function updateCountDown2(amount) {
            ...
            if ((countDown - T3) <= 0) {
                ...
                countDown = T3;
                postChatMessage(":minionwow");
                isCountDownActive = true;
                autoPostActive = true;
                postChatMessage("/topic " + TopicText3);
                postChatMessage(TwitMsg);
                postChatMessage(PMValue + " urmatoral topic: " + TopicText3);
                postChatMessage(VidMsg);
                showCurrentCountDown();
            }
            else {
        ...
        function updateCountDown3(amount) {
            ...
            if ((countDown - T4) <= 0) {
                ...
                isCountDownActive = true;
                autoPostActive = true;
                postChatMessage(":bdfaint");
                postChatMessage("/topic " + TopicText4);
                postChatMessage(TwitMsg);
                postChatMessage(PMValue + " urmatoral topic: " + TopicText4);
                postChatMessage(VidMsg);
                showCurrentCountDown();
            }
            else {
        ...
        function updateCountDown4(amount) {
            ...
            if ((countDown - T5) <= 0) {
                ...
                postChatMessage("/topic " + TopicText5);
                postChatMessage(TwitMsg);
                postChatMessage(PMValue + " urmatoral topic: " + TopicText5);
                postChatMessage(VidMsg);
                showCurrentCountDown();
                postChatMessage(":mclap");
            }
            else {
                showCurrentCountDown();
                //post the count if auto post is active
        ...
        function updateCountDown5(amount) {
            ...
            if (countDown <= 0) {
                ...
                postChatMessage("/topic " + HoldingTopic);
                postChatMessage(TwitMsg);
                postChatMessage(PMValue + " urmatoral topic: " + HoldingTopic);
                postChatMessage(VidMsg);
            }
            else {
                showCurrentCountDown();
        ...
        function modelChanged(modelId) {
            ...
            topiconeRegex = /(!topic1)/i;
            topictwoRegex = /(!topic2)/i;
            topicthreeRegex = /(!topic3)/i;
            topicfourRegex = /(!topic4)/i;
            topicfiveRegex = /(!topic5)/i;
            helpRegex = /(!help)/i;
            postRegex = /(!post)/i;
            stopRegex = /(!stop)/i;
            plusRegex = /(plus (.*))/i;
            minusRegex = /(minus (.*))/i;
            topicRegex = /(topic?)/;
            noRegex = /(anal|skype|baby|bb|group)/i;
            countDown = 0;
            isCountDownActive = false;
            highestTip = 0;
            parsing = false;
